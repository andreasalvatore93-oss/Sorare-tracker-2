...
# cartella, additivo/diagnostico -- vedi formazione_cechia/predict/
...
# formazione_cechia/predict/test_gk.py`, quindi l'import diretto funziona a
...
DISCOVERY_FILE = os.path.join(
    'formazione_cechia/output/cechia_gk_discovery_global' if CALIBRATION_MODE else 'formazione_cechia/output/cechia_gk_discovery',
    'player_slugs.json')
...
OUTPUT_DIR = 'formazione_cechia/output/cechia_gk_calibration' if CALIBRATION_MODE else 'formazione_cechia/output/cechia_gk_all'
...
_CIRCUIT_BREAKER_PATH = '/tmp/sorare_cloudfront_block_cechia_gk.marker'
...
def media_condizionata(values, weights, condition_flags, target_condition, fallback_mean, shrink_k=5.0):
    ...
    cache -- vedi formazione_cechia/diagnostics/inspect_decisive_event_conditioning.py
...
# del riassunto) -- vedi formazione_cechia/predict/test_def.py per la stessa
...
def venue_factor_gk(scores, is_home_flags, target_is_home, weights):
    ...
    non sui residui: verificato il 31/07 (formazione_cechia/diagnostics/
...
def rigorous_backtest_prod_gk(scores, is_home_flags, granulari_values,
                              ...
                              presence_rate=None, league='cechia',
...
def build_prediction(player_slug):
    ...
    if CALIBRATION_MODE and not future_games:
        ...
        grid_results = run_grid_search_prod_gk(
            ...
            opponent_team_slugs_hist=opponent_team_slugs_hist,
            game_dates_hist=game_dates_hist,
            presence_rate=presence_rate, league='cechia',
            detail_ok_flags=detail_ok_flags, goalkeeping_values=goalkeeping_values)
    ...
    # dopo backtest walk-forward rigoroso (formazione_cechia/diagnostics/
    ...
    _opp_lambda_mult = opponent_strength.opponent_lambda_multiplier(
        'cechia', 'gk', next_opponent_team_slug, _opp_cutoff)
    ...
    # -0.13% MAE (formazione_cechia/diagnostics/validate_cross_role_combos.py,
    ...
    # backtest walk-forward rigoroso su dati reali (formazione_cechia/diagnostics/
