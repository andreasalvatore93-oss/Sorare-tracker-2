...
Formula (identica nella struttura, gruppi granulari ampliati):
  score_atteso = P(gioca) x media_pesata_esponenziale(N partite)
                 ...
                 x fattore_azioni_difensive x fattore_gol_subiti x fattore_trend
...
HALF_LIFE_GAMES = 12.0  # PARTENZA (25/07): valore riusato da centrocampisti/attaccanti — da ricalibrare con grid search dedicato ai difensori
RANGE_MULTIPLIER = 1.4  # PARTENZA (25/07): idem
OPPONENT_SENSITIVITY = 29.0  # PARTENZA (25/07): idem
TREND_INTENSITY = 1.0  # PARTENZA (25/07): idem — valore centrocampisti riusato, da ricalibrare con grid search dedicato ai difensori
MIN_MINUTES_PLAYED = 60  # partite giocate sotto questa soglia (subentri) escluse dalla finestra
MIN_STARTER_ODDS = 0.0  # DISATTIVATO (25/07) per la fase di GRID SEARCH sui difensori: il filtro escluderebbe i giocatori PRIMA di girare il backtest, riducendo il campione di calibrazione. Riattivare a 0.70 SOLO quando si passa all'uso reale/produzione (dopo aver fissato i parametri).
...
def build_prediction(player_slug):
    ...
    for node, detail in zip(usable, details):
        ...
        clean_sheet_values.append(extract_group_score(detail, CLEAN_SHEET_STATS))
    ...
    score_atteso = (p_gioca * media_pesata * fattore_casa_trasferta * fattore_forza_avversario
                    ...
                    * fattore_gol_subiti * fattore_clean_sheet * fattore_trend)
    ...
    # Grid search di calibrazione GIA' concluso (19 difensori posseduti,
    ...
    # centrocampisti/attaccanti). Il difensore e' ancora in fase di
