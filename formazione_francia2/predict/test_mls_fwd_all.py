"""
Tool_formazione_mls_fwd_all (test TUTTI gli attaccanti Ligue 2 (Francia) in_season posseduti)

Estende test_multi_fwd.py: invece di una lista statica di 7 giocatori, legge
la lista COMPLETA degli attaccanti Ligue 2 (Francia) in_season posseduti (numero variabile,
generata da francia2_fwd_discovery.py) da francia2_fwd_discovery/player_slugs.json.
...
- Lista giocatori letta dinamicamente da francia2_fwd_discovery/player_slugs.json
...
# GLOBALE (tutti gli attaccanti Ligue 2 (Francia) di qualita', non solo posseduti) invece
...
# Fallback statico SOLO se francia2_fwd_discovery/player_slugs.json non esiste
...
def build_prediction(player_slug):
    ...
    _opp_lambda_mult = opponent_strength.opponent_lambda_multiplier(
        'francia2', 'fwd', next_opponent_team_slug, datetime.datetime.utcnow())
    ...
    _fwd_offense_delta = opponent_strength.fwd_offense_granular_delta(
        'francia2', next_opponent_team_slug, datetime.datetime.utcnow(), _offensive_hist)
    grezzo_nuovo = (level_score_atteso + media_granulari_pesata * fattore_trend_granulare
                    + _fwd_offense_delta)
...
def main():
    ...
    log("Avvio test TUTTI attaccanti Ligue 2 (Francia) in_season Tool_formazione...")
