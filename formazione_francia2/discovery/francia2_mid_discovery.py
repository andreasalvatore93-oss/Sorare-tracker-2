"""
Ligue 2 (Francia) Midfielder Discovery
...
Output: francia2_mid_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_francia2/output/francia2_mid_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [francia2_mid_discovery] {msg}")
...
def discover_francia2_midfielders_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Midfielder posseduti (qualunque lega) e scarta quelli non Francia
    dopo il fetch."""
...
def main():
    log("Avvio discovery centrocampisti Ligue 2 (Francia) (in_season + classic)...")
    slugs, card_counts = discover_francia2_midfielders_all()
