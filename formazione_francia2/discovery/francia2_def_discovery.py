"""
Ligue 2 (Francia) Defender Discovery
...
lega Ligue 2 (Francia) (slug 'ligue-2-fr').
...
mlspa). Non conosciamo il codice equivalente per Francia (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'ligue-2-fr'. Query un po' meno
...
Output: francia2_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_francia2/output/francia2_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Ligue 2 (Francia).
TARGET_LEAGUE_SLUG = 'ligue-2-fr'
...
def log(msg):
    ...
    print(f"[{ts}] [francia2_def_discovery] {msg}")
...
def discover_francia2_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Ligue 2 (Francia)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Francia
    dopo il fetch."""
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Francia applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Francia unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Francia potrebbero essere
...
def main():
    log("Avvio discovery difensori Ligue 2 (Francia) (in_season + classic)...")
    slugs, card_counts = discover_francia2_defenders_all()
