"""
1. HNL (Croazia) Defender Discovery
...
mlspa). Non conosciamo il codice equivalente per Croazia (rischio di
...
si tiene solo chi ha domesticLeague.slug == '1-hnl'. Query un po' meno
...
Output: croazia_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_croazia/output/croazia_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO 1. HNL (Croazia).
...
def log(msg):
    ...
    print(f"[{ts}] [croazia_def_discovery] {msg}")
...
def discover_croazia_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    1. HNL (Croazia)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Croazia
    dopo il fetch."""
    ...
    query = """
    query CroatiaDefenderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="CroatiaDefenderDiscovery")
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Croazia applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Croazia unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Croazia potrebbero essere
...
def main():
    log("Avvio discovery difensori 1. HNL (Croazia) (in_season + classic)...")
    slugs, card_counts = discover_croazia_defenders_all()
