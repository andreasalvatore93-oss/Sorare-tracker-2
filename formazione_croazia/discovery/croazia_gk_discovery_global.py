"""
croazia_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri HNL (Croazia) in_season attivi (NON solo
...
resta sui soli giocatori posseduti via croazia_gk_discovery.py).

Clone di mls_gk_discovery_global.py, stesso approccio: per ognuna delle 10
...
Output: croazia_gk_discovery_global/player_slugs.json -- lista JSON di slug
giocatore portiere unici, deduplicati su tutte le 10 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_croazia/output/croazia_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [croazia_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri HNL (Croazia) su {len(CROAZIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri HNL (Croazia) unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri HNL (Croazia) dopo filtro qualita': {len(slugs)}")
