"""
croazia_mid_discovery_global.py

Discovery GLOBALE di tutti i centrocampisti HNL (Croazia) in_season attivi (NON solo
...
resta sui soli giocatori posseduti via croazia_mid_discovery.py).

Clone di mls_mid_discovery_global.py, stesso approccio: per ognuna delle 10
...
Output: croazia_mid_discovery_global/player_slugs.json -- lista JSON di slug
giocatore centrocampiste unici, deduplicati su tutte le 10 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_croazia/output/croazia_mid_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [croazia_mid_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE centrocampisti HNL (Croazia) su {len(CROAZIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale centrocampisti HNL (Croazia) unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale centrocampisti HNL (Croazia) dopo filtro qualita': {len(slugs)}")
