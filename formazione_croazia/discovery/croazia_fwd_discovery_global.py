"""
croazia_fwd_discovery_global.py
...
resta sui soli giocatori posseduti via croazia_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 10
squadre HNL (Croazia) interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "1-hnl") { clubs(first: 50) { nodes {
slug name } } } }` -- 10/10 squadre restituite (workflow
...
Output: croazia_fwd_discovery_global/player_slugs.json -- lista JSON di slug
giocatore attaccante unici, deduplicati su tutte le 10 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_croazia/output/croazia_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "1-hnl")
# .clubs, 31/07) -- 10/10
CROAZIA_TEAM_SLUGS = [
    ...
    'hajduk-split-split',
    'istra-1961-pula',
    'lokomotiva-zagreb-zagreb',
    'osijek-osijek',
    'rijeka-rijeka',
    'rudes-zagreb',
    'slaven-koprivnica-koprivnica',
    'varazdin-varazdin',
]
...
def log(msg):
    ...
    print(f"[{ts}] [croazia_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti HNL (Croazia) su {len(CROAZIA_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(CROAZIA_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(CROAZIA_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti HNL (Croazia) unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti HNL (Croazia) dopo filtro qualita': {len(slugs)}")
