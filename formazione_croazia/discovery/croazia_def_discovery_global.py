"""
croazia_def_discovery_global.py

Discovery GLOBALE di tutti i difensori HNL (Croazia) in_season attivi (NON solo
...
resta sui soli giocatori posseduti via croazia_def_discovery.py).

Clone di mls_def_discovery_global.py, stesso approccio: per ognuna delle 10
...
Output: croazia_def_discovery_global/player_slugs.json -- lista JSON di slug
giocatore difensore unici, deduplicati su tutte le 10 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_croazia/output/croazia_def_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [croazia_def_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE difensori HNL (Croazia) su {len(CROAZIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale difensori HNL (Croazia) unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale difensori HNL (Croazia) dopo filtro qualita': {len(slugs)}")
