"""
1. HNL (Croazia) Goalkeeper Discovery
...
Output: croazia_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_croazia/output/croazia_gk_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [croazia_gk_discovery] {msg}")
...
def discover_croazia_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Croazia
    dopo il fetch."""
    ...
    query = """
    query CroatiaGoalkeeperDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="CroatiaGoalkeeperDiscovery")
...
def main():
    log("Avvio discovery portieri 1. HNL (Croazia) (in_season + classic)...")
    slugs, card_counts = discover_croazia_goalkeepers_all()
