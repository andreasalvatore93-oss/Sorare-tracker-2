"""
spagna_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri LaLiga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via spagna_gk_discovery.py).
...
Output: spagna_gk_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_spagna/output/spagna_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [spagna_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri LaLiga su {len(SPAGNA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri LaLiga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri LaLiga dopo filtro qualita': {len(slugs)}")
