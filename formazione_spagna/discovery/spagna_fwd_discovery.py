"""
LaLiga (Spagna) Forward Discovery
...
Output: spagna_fwd_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_spagna/output/spagna_fwd_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [spagna_fwd_discovery] {msg}")
...
def discover_spagna_forwards_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Forward posseduti (qualunque lega) e scarta quelli non Spagna
    dopo il fetch."""
    ...
    query = """
    query SpainForwardDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="SpainForwardDiscovery")
...
def main():
    log("Avvio discovery attaccanti LaLiga (Spagna) (in_season + classic)...")
    slugs, card_counts = discover_spagna_forwards_all()
