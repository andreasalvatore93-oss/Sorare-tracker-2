"""
LaLiga (Spagna) Goalkeeper Discovery
...
Output: spagna_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_spagna/output/spagna_gk_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [spagna_gk_discovery] {msg}")
...
def discover_spagna_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Spagna
    dopo il fetch."""
    ...
    query = """
    query SpainGoalkeeperDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="SpainGoalkeeperDiscovery")
...
def main():
    log("Avvio discovery portieri LaLiga (Spagna) (in_season + classic)...")
    slugs, card_counts = discover_spagna_goalkeepers_all()
