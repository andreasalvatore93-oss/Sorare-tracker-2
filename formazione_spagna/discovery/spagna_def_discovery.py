"""
LaLiga (Spagna) Defender Discovery
...
lega LaLiga (Spagna) (slug 'laliga-es').
...
mlspa). Non conosciamo il codice equivalente per Spagna (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'laliga-es'. Query un po' meno
...
Output: spagna_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_spagna/output/spagna_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO LaLiga (Spagna).
TARGET_LEAGUE_SLUG = 'laliga-es'
...
def log(msg):
    ...
    print(f"[{ts}] [spagna_def_discovery] {msg}")
...
def discover_spagna_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    LaLiga (Spagna)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Spagna
    dopo il fetch."""
    ...
    query = """
    query SpainDefenderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="SpainDefenderDiscovery")
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Spagna applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Spagna unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Spagna potrebbero essere
...
def main():
    log("Avvio discovery difensori LaLiga (Spagna) (in_season + classic)...")
    slugs, card_counts = discover_spagna_defenders_all()
