"""
Liga Profesional (Argentina) Midfielder Discovery
...
Output: argentina_mid_discovery/player_slugs.json -- lista JSON di slug
...
def log(msg):
    ...
    print(f"[{ts}] [argentina_mid_discovery] {msg}")
...
def discover_argentina_midfielders_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Midfielder posseduti (qualunque lega) e scarta quelli non Argentina
    dopo il fetch."""
...
def main():
    log("Avvio discovery centrocampisti Liga Profesional (Argentina) (in_season + classic)...")
    slugs, card_counts = discover_argentina_midfielders_all()
