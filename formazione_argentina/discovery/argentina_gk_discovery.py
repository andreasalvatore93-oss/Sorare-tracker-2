"""
Liga Profesional (Argentina) Goalkeeper Discovery
...
Output: argentina_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_argentina/output/argentina_gk_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [argentina_gk_discovery] {msg}")
...
def discover_argentina_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Argentina
    dopo il fetch."""
...
def main():
    log("Avvio discovery portieri Liga Profesional (Argentina) (in_season + classic)...")
    slugs, card_counts = discover_argentina_goalkeepers_all()
