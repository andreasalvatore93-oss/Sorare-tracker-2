"""
argentina_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri Liga Profesional Argentina in_season attivi (NON solo
...
resta sui soli giocatori posseduti via argentina_gk_discovery.py).

Clone di mls_gk_discovery_global.py, stesso approccio: per ognuna delle 30
squadre Liga Profesional Argentina interroga Club(slug).anyPlayers per il roster completo,
poi filtra lato client per posizione Goalkeeper (anyPositions del
...
Output: argentina_gk_discovery_global/player_slugs.json -- lista JSON di slug
giocatore portiere unici, deduplicati su tutte le 18 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_argentina/output/argentina_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [argentina_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri Liga Profesional Argentina su {len(ARGENTINA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri Liga Profesional Argentina unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri Liga Profesional Argentina dopo filtro qualita': {len(slugs)}")
