"""
Liga Profesional (Argentina) Defender Discovery
...
lega Liga Profesional (Argentina) (slug 'superliga-argentina-de-futbol').
...
mlspa). Non conosciamo il codice equivalente per Argentina (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'superliga-argentina-de-futbol'. Query un po' meno
...
Output: argentina_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_argentina/output/argentina_def_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [argentina_def_discovery] {msg}")
...
def discover_argentina_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Liga Profesional (Argentina)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Argentina
    dopo il fetch."""
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Argentina applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Argentina unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Argentina potrebbero essere
...
def main():
    log("Avvio discovery difensori Liga Profesional (Argentina) (in_season + classic)...")
    slugs, card_counts = discover_argentina_defenders_all()
