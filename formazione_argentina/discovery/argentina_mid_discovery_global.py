"""
argentina_mid_discovery_global.py

Discovery GLOBALE di tutti i centrocampisti Liga Profesional Argentina in_season attivi (NON solo
...
resta sui soli giocatori posseduti via argentina_mid_discovery.py).
...
Output: argentina_mid_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_argentina/output/argentina_mid_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [argentina_mid_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE centrocampisti Liga Profesional Argentina su {len(ARGENTINA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale centrocampisti Liga Profesional Argentina unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale centrocampisti Liga Profesional Argentina dopo filtro qualita': {len(slugs)}")
