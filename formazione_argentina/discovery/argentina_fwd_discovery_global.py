"""
argentina_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti Liga Profesional Argentina in_season attivi (NON solo
...
resta sui soli giocatori posseduti via argentina_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 30
squadre Liga Profesional Argentina interroga Club(slug).anyPlayers per il roster completo,
poi filtra lato client per posizione Forward (anyPositions del
giocatore). Nessuno scope utente richiesto: query pubblica.

Slug squadre ottenuti dal vivo (31/07) con la query pubblica
`football { competition(slug: "superliga-argentina-de-futbol") { clubs(first: 50) { nodes {
slug name } } } }` -- 30/30 squadre restituite (workflow
verify_clubs.yml, run 30621182980).
...
Output: argentina_fwd_discovery_global/player_slugs.json -- lista JSON di slug
giocatore attaccante unici, deduplicati su tutte le 18 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_argentina/output/argentina_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "superliga-argentina-de-futbol")
# .clubs, 31/07) -- 30/30
ARGENTINA_TEAM_SLUGS = [
    'aldosivi-mar-del-plata-provincia-de-buenos-aires',
    'argentinos-juniors-capital-federal-ciudad-de-buenos-aires',
    'atletico-tucuman-san-miguel-de-tucuman-provincia-de-tucuman',
    'banfield-lomas-de-zamora-provincia-de-buenos-aires',
    'barracas-central-capital-federal-ciudad-de-buenos-aires',
    'belgrano-ciudad-de-cordoba-provincia-de-cordoba',
    'boca-juniors-capital-federal-ciudad-de-buenos-aires',
    ...
    'deportivo-riestra-capital-federal-ciudad-de-buenos-aires',
    'estudiantes-la-plata-provincia-de-buenos-aires',
    'estudiantes-rio-cuarto-rio-cuarto-provincia-de-cordoba',
    'gimnasia-la-plata-la-plata-provincia-de-buenos-aires',
    'gimnasia-mendoza-mendoza-provincia-de-mendoza',
    'huracan-capital-federal-ciudad-de-buenos-aires',
    'independiente-avellaneda-provincia-de-buenos-aires',
    'independiente-rivadavia-mendoza-provincia-de-mendoza',
    'instituto-ciudad-de-cordoba-provincia-de-cordoba',
    'lanus-lanus-provincia-de-buenos-aires',
    'newell-s-old-boys-rosario-santa-fe',
    'platense-vicente-lopez-provincia-de-buenos-aires',
    'racing-club-avellaneda-provincia-de-buenos-aires',
    'river-plate-capital-federal-ciudad-de-buenos-aires',
    'rosario-central-rosario-provincia-de-santa-fe',
    'san-lorenzo-capital-federal-ciudad-de-buenos-aires',
    'sarmiento-junin-provincia-de-buenos-aires',
    'talleres-cordoba-ciudad-de-cordoba-provincia-de-cordoba',
    'tigre-victoria-provincia-de-buenos-aires',
    'union-santa-fe-ciudad-de-santa-fe-provincia-de-santa-fe',
    'velez-sarsfield-capital-federal-ciudad-de-buenos-aires',
]
...
def log(msg):
    ...
    print(f"[{ts}] [argentina_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti Liga Profesional Argentina su {len(ARGENTINA_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(ARGENTINA_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(ARGENTINA_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti Liga Profesional Argentina unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti Liga Profesional Argentina dopo filtro qualita': {len(slugs)}")
