"""
argentina_def_discovery_global.py

Discovery GLOBALE di tutti i difensori Liga Profesional Argentina in_season attivi (NON solo
...
resta sui soli giocatori posseduti via argentina_def_discovery.py).

Clone di mls_def_discovery_global.py, stesso approccio: per ognuna delle 30
...
Output: argentina_def_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_argentina/output/argentina_def_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [argentina_def_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE difensori Liga Profesional Argentina su {len(ARGENTINA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale difensori Liga Profesional Argentina unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale difensori Liga Profesional Argentina dopo filtro qualita': {len(slugs)}")
