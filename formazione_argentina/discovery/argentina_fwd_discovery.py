"""
Liga Profesional (Argentina) Forward Discovery
...
Output: argentina_fwd_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_argentina/output/argentina_fwd_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [argentina_fwd_discovery] {msg}")
...
def discover_argentina_forwards_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Forward posseduti (qualunque lega) e scarta quelli non Argentina
    dopo il fetch."""
...
def main():
    log("Avvio discovery attaccanti Liga Profesional (Argentina) (in_season + classic)...")
    slugs, card_counts = discover_argentina_forwards_all()
