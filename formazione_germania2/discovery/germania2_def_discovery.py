"""
2. Bundesliga (Germania) Defender Discovery
...
lega 2. Bundesliga (Germania) (slug '2-bundesliga').
...
mlspa). Non conosciamo il codice equivalente per Germania (rischio di
...
si tiene solo chi ha domesticLeague.slug == '2-bundesliga'. Query un po' meno
...
Output: germania2_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_germania2/output/germania2_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO 2. Bundesliga (Germania).
TARGET_LEAGUE_SLUG = '2-bundesliga'
...
def log(msg):
    ...
    print(f"[{ts}] [germania2_def_discovery] {msg}")
...
def discover_germania2_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    2. Bundesliga (Germania)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Germania
    dopo il fetch."""
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Germania applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Germania unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Germania potrebbero essere
...
def main():
    log("Avvio discovery difensori 2. Bundesliga (Germania) (in_season + classic)...")
    slugs, card_counts = discover_germania2_defenders_all()
