"""
germania2_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti 2. Bundesliga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via germania2_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 18
squadre 2. Bundesliga interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "2-bundesliga") { clubs(first: 50) { nodes {
...
Output: germania2_fwd_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_germania2/output/germania2_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "2-bundesliga")
# .clubs, 31/07) -- 18/18
GERMANIA2_TEAM_SLUGS = [
...
def log(msg):
    ...
    print(f"[{ts}] [germania2_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti 2. Bundesliga su {len(GERMANIA2_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(GERMANIA2_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(GERMANIA2_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti 2. Bundesliga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti 2. Bundesliga dopo filtro qualita': {len(slugs)}")
