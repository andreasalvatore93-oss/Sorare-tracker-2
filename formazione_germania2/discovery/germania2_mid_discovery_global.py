"""
germania2_mid_discovery_global.py

Discovery GLOBALE di tutti i centrocampisti 2. Bundesliga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via germania2_mid_discovery.py).
...
Output: germania2_mid_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_germania2/output/germania2_mid_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [germania2_mid_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE centrocampisti 2. Bundesliga su {len(GERMANIA2_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale centrocampisti 2. Bundesliga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale centrocampisti 2. Bundesliga dopo filtro qualita': {len(slugs)}")
