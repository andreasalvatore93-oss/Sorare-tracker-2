"""
francia_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti Ligue 1 in_season attivi (NON solo
...
resta sui soli giocatori posseduti via francia_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 18
squadre Ligue 1 interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "ligue-1-fr") { clubs(first: 50) { nodes {
...
Output: francia_fwd_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_francia/output/francia_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "ligue-1-fr")
# .clubs, 31/07) -- 18/18
FRANCIA_TEAM_SLUGS = [
    'angers-sco-angers',
    'auxerre-auxerre',
...
def log(msg):
    ...
    print(f"[{ts}] [francia_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti Ligue 1 su {len(FRANCIA_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(FRANCIA_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(FRANCIA_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti Ligue 1 unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti Ligue 1 dopo filtro qualita': {len(slugs)}")
