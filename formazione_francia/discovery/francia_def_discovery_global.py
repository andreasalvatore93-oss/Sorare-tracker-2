"""
francia_def_discovery_global.py

Discovery GLOBALE di tutti i difensori Ligue 1 in_season attivi (NON solo
...
resta sui soli giocatori posseduti via francia_def_discovery.py).
...
Output: francia_def_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_francia/output/francia_def_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [francia_def_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE difensori Ligue 1 su {len(FRANCIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale difensori Ligue 1 unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale difensori Ligue 1 dopo filtro qualita': {len(slugs)}")
