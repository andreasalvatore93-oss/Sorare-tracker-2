"""
Ligue 1 (Francia) Forward Discovery
...
Output: francia_fwd_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_francia/output/francia_fwd_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [francia_fwd_discovery] {msg}")
...
def discover_francia_forwards_all(user_slug=USER_SLUG, max_pages=50):
...
def main():
    log("Avvio discovery attaccanti Ligue 1 (Francia) (in_season + classic)...")
    slugs, card_counts = discover_francia_forwards_all()
