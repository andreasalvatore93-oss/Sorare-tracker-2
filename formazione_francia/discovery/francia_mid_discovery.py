"""
Ligue 1 (Francia) Midfielder Discovery
...
Output: francia_mid_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_francia/output/francia_mid_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [francia_mid_discovery] {msg}")
...
def discover_francia_midfielders_all(user_slug=USER_SLUG, max_pages=50):
...
def main():
    log("Avvio discovery centrocampisti Ligue 1 (Francia) (in_season + classic)...")
    slugs, card_counts = discover_francia_midfielders_all()
