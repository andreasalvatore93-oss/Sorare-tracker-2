"""
Ligue 1 (Francia) Defender Discovery
...
lega Ligue 1 (Francia) (slug 'ligue-1-fr').
...
si tiene solo chi ha domesticLeague.slug == 'ligue-1-fr'. Query un po' meno
...
Output: francia_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_francia/output/francia_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Ligue 1 (Francia).
TARGET_LEAGUE_SLUG = 'ligue-1-fr'
...
def log(msg):
    ...
    print(f"[{ts}] [francia_def_discovery] {msg}")
...
def discover_francia_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Ligue 1 (Francia)) posseduti da user_slug, SIA in_season CHE classic. Filtro
...
def main():
    log("Avvio discovery difensori Ligue 1 (Francia) (in_season + classic)...")
    slugs, card_counts = discover_francia_defenders_all()
