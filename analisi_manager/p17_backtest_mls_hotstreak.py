...
# ---------------------------------------------------------------------------
# BASE PULITA (07/08/2026, decisa dall'utente riga per riga). Si tengono SOLO
# le formazioni che superano tutti questi controlli, perche' sono le uniche di
# cui ci si puo' fidare:
#   - competizione All Star o Under 23 -> stessa forma da 7, stesso bonus
#     capitano +50%, nessun limite di carte classic. Sono identiche fra loro:
#     cambia solo chi e' eleggibile, quindi si analizzano INSIEME. Tutte le
#     altre competizioni sono escluse perche' per ricostruirle servirebbe
#     sapere quali carte erano in season ALLORA, e il campo in_season nei file
#     e' letto al momento dell'estrazione (ricostruisci_manager.py:279): dice
#     se la carta e' in season OGGI, non allora.
#   - tutte le carte di rarita' 'limited': nelle rare/super rare/unique il
#     bonus xp e' amplificato (rispettivamente +20/+50/+100 per carta), quindi
#     i punteggi non sono confrontabili.
#   - somma dei punteggi delle carte = punteggio UFFICIALE di Sorare entro
#     0.5. E' la verifica che non dipende da nessuna teoria sui bonus: se
#     coincide, il dato e' quello vero. Ne cadono 36 su 1186.
#   - leaderboard con 'division-N': le formazioni senza division non
#     corrispondono a nessuna competizione reale (confermato dall'utente).
# Risultato: 1004 formazioni (658 All Star + 346 Under 23), 7028 carte tutte
# limited, 7 carte e 1 capitano ciascuna, zero arene.
def _base_pulita(f):
    """BASE PULITA MLS HOT STREAK (07/08/2026, criteri decisi dall'utente uno
    per uno e verificati sui dati prima di misurare qualunque cosa):
      - competizione 'MLS - Hot Streak - Limited', 5 carte, una sola
        leaderboard reale (seasonal-us-in_season_us_limited_pve, nessuna
        division: confermato dall'utente);
      - tutte le carte di rarita' 'limited' -- nelle rare/super rare/unique il
        bonus xp e' amplificato e i punteggi non sarebbero confrontabili;
      - somma dei punteggi delle carte = punteggio UFFICIALE di Sorare (scarto
        massimo misurato 0.0098). E' la verifica che non dipende da nessuna
        teoria sui bonus: se coincide, il dato e' quello vero;
      - AL MASSIMO 1 carta classic: e' la regola della competizione
        (MLS_IN_SEASON, max_classic=1). Le 6 formazioni con 2 classic sono
        escluse -- contenevano Griezmann e Lewandowski, che in MLS non giocano.
    Risultato: 314 formazioni, 1570 carte tutte limited, zero arene, un
    capitano ciascuna (+50%), xp presente su 1568 carte."""
    comp = (f.get('competizione') or '').lower().replace('–', '-')
    if comp != 'mls - hot streak - limited':
        return False
    carte = f.get('carte') or []
    if len(carte) != 5:
        return False
    if not all((c.get('rarita') or '').lower() == 'limited' for c in carte):
        return False
    uff = (f.get('piazzamento') or {}).get('punteggio')
    if uff is None:
        return False
    if abs(sum(c.get('punteggio') or 0 for c in carte) - uff) >= 0.5:
        return False
    return sum(1 for c in carte if c.get('in_season') is False) <= 1


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--limit-manager', type=int, default=None)
    ap.add_argument('--dump', default='analisi_manager/p17_dump_esempio.txt')
    args = ap.parse_args()
    ...
    for fp in files:
        ...
        for gw, formazioni in gws.items():
            ...
            non_arene = [f for f in formazioni
                         if not f.get('tipo_arena') and f.get('carte') and _base_pulita(f)]
            if not non_arene:
                continue
    ...
    with open('analisi_manager/p17_backtest_mls_out.json', 'w', encoding='utf-8') as fh:
        json.dump({
    ...
    print('\nsalvato analisi_manager/p17_backtest_mls_out.json')
