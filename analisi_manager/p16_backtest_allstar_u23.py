...
def _base_pulita(f):
    comp = (f.get('competizione') or '').lower()
    if 'all star' not in comp and 'under 23' not in comp:
        return False
    ...
    if abs(sum(c.get('punteggio') or 0 for c in carte) - uff) >= 0.5:
        return False
    return bool(re.search(r'division-[0-9]', f.get('leaderboard') or ''))


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--limit-manager', type=int, default=None)
    ap.add_argument('--dump', default='analisi_manager/p16_dump_esempio.txt')
    args = ap.parse_args()
    ...
    with open('analisi_manager/p16_backtest_allstar_u23_out.json', 'w', encoding='utf-8') as fh:
        json.dump({
    ...
    print('\nsalvato analisi_manager/p16_backtest_allstar_u23_out.json')
