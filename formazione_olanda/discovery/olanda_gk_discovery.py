"""
Eredivisie (Olanda) Goalkeeper Discovery
...
Output: olanda_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_olanda/output/olanda_gk_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [olanda_gk_discovery] {msg}")
...
def discover_olanda_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Olanda
    dopo il fetch."""
    ...
    query = """
    query NetherlandsGoalkeeperDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="NetherlandsGoalkeeperDiscovery")
...
def main():
    log("Avvio discovery portieri Eredivisie (Olanda) (in_season + classic)...")
    slugs, card_counts = discover_olanda_goalkeepers_all()
