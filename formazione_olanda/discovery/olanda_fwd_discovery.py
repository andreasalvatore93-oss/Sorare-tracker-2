"""
Eredivisie (Olanda) Forward Discovery
...
Output: olanda_fwd_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_olanda/output/olanda_fwd_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [olanda_fwd_discovery] {msg}")
...
def discover_olanda_forwards_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Forward posseduti (qualunque lega) e scarta quelli non Olanda
    dopo il fetch."""
    ...
    query = """
    query NetherlandsForwardDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="NetherlandsForwardDiscovery")
...
def main():
    log("Avvio discovery attaccanti Eredivisie (Olanda) (in_season + classic)...")
    slugs, card_counts = discover_olanda_forwards_all()
