"""
Eredivisie (Olanda) Midfielder Discovery
...
Output: olanda_mid_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_olanda/output/olanda_mid_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [olanda_mid_discovery] {msg}")
...
def discover_olanda_midfielders_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Midfielder posseduti (qualunque lega) e scarta quelli non Olanda
    dopo il fetch."""
    ...
    query = """
    query NetherlandsMidfielderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="NetherlandsMidfielderDiscovery")
...
def main():
    log("Avvio discovery centrocampisti Eredivisie (Olanda) (in_season + classic)...")
    slugs, card_counts = discover_olanda_midfielders_all()
