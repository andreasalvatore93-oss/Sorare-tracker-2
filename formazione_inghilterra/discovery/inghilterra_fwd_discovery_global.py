"""
inghilterra_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti Premier League in_season attivi (NON solo
...
resta sui soli giocatori posseduti via inghilterra_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 20
squadre Premier League interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "premier-league-gb-eng") { clubs(first: 50) { nodes {
slug name } } } }` -- 20/20 squadre restituite (verifica dal vivo 31/07).
...
Output: inghilterra_fwd_discovery_global/player_slugs.json -- lista JSON di slug
giocatore attaccante unici, deduplicati su tutte le 20 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_inghilterra/output/inghilterra_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "premier-league-gb-eng")
# .clubs, 31/07) -- 20/20
INGHILTERRA_TEAM_SLUGS = [
...
def log(msg):
    ...
    print(f"[{ts}] [inghilterra_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti Premier League su {len(INGHILTERRA_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(INGHILTERRA_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(INGHILTERRA_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti Premier League unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti Premier League dopo filtro qualita': {len(slugs)}")
