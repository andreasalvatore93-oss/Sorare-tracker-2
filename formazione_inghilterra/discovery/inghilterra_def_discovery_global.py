"""
inghilterra_def_discovery_global.py

Discovery GLOBALE di tutti i difensori Premier League in_season attivi (NON solo
...
resta sui soli giocatori posseduti via inghilterra_def_discovery.py).

Clone di mls_def_discovery_global.py, stesso approccio: per ognuna delle 20
...
Output: inghilterra_def_discovery_global/player_slugs.json -- lista JSON di slug
giocatore difensore unici, deduplicati su tutte le 20 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_inghilterra/output/inghilterra_def_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [inghilterra_def_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE difensori Premier League su {len(INGHILTERRA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale difensori Premier League unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale difensori Premier League dopo filtro qualita': {len(slugs)}")
