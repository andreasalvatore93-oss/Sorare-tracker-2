"""
inghilterra_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri Premier League in_season attivi (NON solo
...
resta sui soli giocatori posseduti via inghilterra_gk_discovery.py).

Clone di mls_gk_discovery_global.py, stesso approccio: per ognuna delle 20
...
Output: inghilterra_gk_discovery_global/player_slugs.json -- lista JSON di slug
giocatore portiere unici, deduplicati su tutte le 20 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_inghilterra/output/inghilterra_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [inghilterra_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri Premier League su {len(INGHILTERRA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri Premier League unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri Premier League dopo filtro qualita': {len(slugs)}")
