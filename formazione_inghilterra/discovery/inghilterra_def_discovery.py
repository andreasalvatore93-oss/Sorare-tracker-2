"""
Premier League (Inghilterra) Defender Discovery
...
lega Premier League (Inghilterra) (slug 'premier-league-gb-eng').
...
si tiene solo chi ha domesticLeague.slug == 'premier-league-gb-eng'. Query un po' meno
...
Output: inghilterra_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_inghilterra/output/inghilterra_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Premier League (Inghilterra).
TARGET_LEAGUE_SLUG = 'premier-league-gb-eng'
...
def log(msg):
    ...
    print(f"[{ts}] [inghilterra_def_discovery] {msg}")
...
def discover_inghilterra_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Premier League (Inghilterra)) posseduti da user_slug, SIA in_season CHE classic. Filtro
...
def main():
    log("Avvio discovery difensori Premier League (Inghilterra) (in_season + classic)...")
    slugs, card_counts = discover_inghilterra_defenders_all()
