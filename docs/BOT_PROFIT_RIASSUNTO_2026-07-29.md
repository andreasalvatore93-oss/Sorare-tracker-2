...
- **61 giocatori aggiunti a blacklist manuale** (`blacklist_manuale`, TTL 1 anno) in 3 giri di revisione dell'utente sull'artifact (10 + 43 + 8, nessun duplicato) — gia' committati e pushati su main. Lista completa in `sorare_lista_nera_profit.txt` (motivo `blacklist_manuale`, scadenza 2027-07-29).
- **Sessione conclusa 29/07**: nessun branch di test residuo (tutti i branch `test/...` usati per i dump/verifiche sono stati eliminati sia locale che remoto a fine uso), working tree pulito, tutto pushato su main.
- **BUG trovato E CORRETTO nello script di dump del roster** (`scanners/_tmp_roster_dump.py`, temporaneo, non nel bot, gia' rimosso da main dopo l'uso): la prima versione escludeva dalla lista di revisione ANCHE i giocatori temporaneamente blacklistati per `prezzo_basso_o_senza_annunci` (TTL 2 giorni, dal TTL introdotto in sezione 2) invece di escludere solo chi era gia' deciso permanentemente (`blacklist_manuale`) — con 503 giocatori in quella blacklist temporanea su tutte le leghe al momento del primo dump, il roster Eredivisie/Belgio risultava tagliato a soli 190 nomi (l'utente ha giustamente notato che sembravano pochi per 36 squadre). **Fix applicato ed eseguito lo stesso giorno**: rigenerato il dump filtrando solo su `blacklist_manuale` -> **494 giocatori unici** (contro i 190 iniziali), artifact ripubblicato sullo STESSO URL con la lista corretta (i 10 gia' decisi in precedenza non compaiono piu'). File temporanei rimossi da main, branch di test eliminato.
