"""
Live prediction log (26/07) -- monitoraggio MAE live per i 4 ruoli Super League (Svizzera).
...
su dati storici in cache (formazione_azerbaigian/diagnostics/validate_*.py). Non
c'e' nessun tracciamento di quanto le predizioni REALI di produzione (quelle
scritte nei file .txt in azerbaigian_<ruolo>_all/) si rivelino accurate una volta
...
formazione_azerbaigian/diagnostics/resolve_live_predictions.py si occupa in seguito
...
Uso (da ciascuno dei 4 script predict, SOLO se non CALIBRATION_MODE):
    ...
    from formazione_azerbaigian.predict.live_prediction_log import log_live_prediction
