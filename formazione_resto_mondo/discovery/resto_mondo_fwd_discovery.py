"""
Resto del Mondo Forward Discovery
...
Output: resto_mondo_fwd_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_resto_mondo/output/resto_mondo_fwd_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [resto_mondo_fwd_discovery] {msg}")
...
def discover_resto_mondo_fwd_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Forward, eleggibile
    ...
    Forward posseduti (qualunque eleggibilita') e scarta quelli non idonei
    ...
    query = """
    query RestoMondoForwardDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="RestoMondoForwardDiscovery")

            search = ((data.get('data') or {}).get('user') or {}).get('searchCards') or {}
            hits = search.get('hits') or []
            if page == 1:
                log(f"Totale carte Forward/{type_key} (TUTTE le eleggibilita') trovate: {search.get('nbHits')} "
...
def main():
    log("Avvio discovery attaccanti Resto del Mondo (in_season + classic)...")
    slugs, card_counts = discover_resto_mondo_fwd_all()
