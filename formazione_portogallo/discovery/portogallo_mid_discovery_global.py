"""
portogallo_mid_discovery_global.py

Discovery GLOBALE di tutti i centrocampisti Primeira Liga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via portogallo_mid_discovery.py).
...
Output: portogallo_mid_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_portogallo/output/portogallo_mid_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [portogallo_mid_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE centrocampisti Primeira Liga su {len(PORTOGALLO_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale centrocampisti Primeira Liga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale centrocampisti Primeira Liga dopo filtro qualita': {len(slugs)}")
