"""
portogallo_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri Primeira Liga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via portogallo_gk_discovery.py).

Clone di mls_gk_discovery_global.py, stesso approccio: per ognuna delle 18
...
Output: portogallo_gk_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_portogallo/output/portogallo_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [portogallo_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri Primeira Liga su {len(PORTOGALLO_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri Primeira Liga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri Primeira Liga dopo filtro qualita': {len(slugs)}")
