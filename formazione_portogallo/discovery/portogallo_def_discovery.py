"""
Primeira Liga (Portogallo) Defender Discovery
...
lega Primeira Liga (Portogallo) (slug 'primeira-liga-pt').
...
mlspa). Non conosciamo il codice equivalente per Portogallo (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'primeira-liga-pt'. Query un po' meno
...
Output: portogallo_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_portogallo/output/portogallo_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Primeira Liga (Portogallo).
TARGET_LEAGUE_SLUG = 'primeira-liga-pt'
...
def log(msg):
    ...
    print(f"[{ts}] [portogallo_def_discovery] {msg}")
...
def discover_portogallo_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Primeira Liga (Portogallo)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Portogallo
    dopo il fetch."""
    ...
    query = """
    query PortugalDefenderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="PortugalDefenderDiscovery")
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Portogallo applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Portogallo unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Portogallo potrebbero essere
...
def main():
    log("Avvio discovery difensori Primeira Liga (Portogallo) (in_season + classic)...")
    slugs, card_counts = discover_portogallo_defenders_all()
