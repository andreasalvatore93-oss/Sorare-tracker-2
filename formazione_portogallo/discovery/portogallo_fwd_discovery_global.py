"""
portogallo_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti Primeira Liga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via portogallo_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 18
squadre Primeira Liga interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "primeira-liga-pt") { clubs(first: 50) { nodes {
slug name } } } }` -- 18/18 squadre restituite (workflow
verify_clubs.yml, run 30619140320).
...
Output: portogallo_fwd_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_portogallo/output/portogallo_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "primeira-liga-pt")
# .clubs, 31/07) -- 18/18
PORTOGALLO_TEAM_SLUGS = [
    ...
    'maritimo-funchal-ilha-da-madeira',
...
def log(msg):
    ...
    print(f"[{ts}] [portogallo_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti Primeira Liga su {len(PORTOGALLO_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(PORTOGALLO_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(PORTOGALLO_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti Primeira Liga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti Primeira Liga dopo filtro qualita': {len(slugs)}")
