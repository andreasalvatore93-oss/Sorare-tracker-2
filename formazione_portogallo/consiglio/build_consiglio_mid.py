"""
Genera SOLO il riepilogo finale compatto "Consiglio centrocampisti" leggendo
...
OUTPUT_DIR = 'formazione_portogallo/output/portogallo_mid_all'
# CONSIGLIO_DISCOVERY_FILE (31/07, stessa patch propagata ad austria/croazia/
# germania2/scozia -- vedi commit 1db59630ba): permette a best_five.py di
# puntare al pool GLOBALE (i sopravvissuti al prefiltro, non i posseduti)
# senza duplicare qui la logica di parsing/sort gia' scritta in questo file.
# Se non impostata, comportamento INVARIATO (posseduti, come sempre).
DISCOVERY_FILE = os.environ.get('CONSIGLIO_DISCOVERY_FILE') or os.path.join('formazione_portogallo/output/portogallo_mid_discovery', 'player_slugs.json')
...
# scritta subito dopo la riga consiglio da test_mid.py -- portata fino a
...
def main():
    ...
    lines = []
    lines.append(f"Consiglio centrocampisti — {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M')}Z")
    lines.append("")
