...
# Fallback statico SOLO se portogallo_def_discovery/player_slugs.json non esiste
...
def fetch_game_log_incremental(slug, target_window_size):
    ...
    # dalla squadra vecchia, sez. "SQUADRA sbagliata/AVVERSARIO N/D" -- vedi
    # RIASSUNTO): activeClub e' SEMPRE live (stesso campo gia' verificato in uso
    # in discovery_fixture.py), zero query aggiuntive (stesso payload). Usato
    # sotto SOLO per determinare la squadra nella PROSSIMA partita (dove serve
    # il dato attuale, non storico) -- il fattore casa/trasferta storico
    # continua a usare la maggioranza sulle partite passate, invariato.
...
# Pattern identico a formazione_mls/predict/test_def.py (vedi commenti li'
...
def build_prediction(player_slug):
    ...
    # essere sbagliata dopo un trasferimento recente (poche/nessuna partita
    # ancora giocata per la nuova squadra). Se activeClub manca o non
    # corrisponde a nessuna delle due squadre della partita target (dato
    # incompleto), ripiega sulla vecchia logica (nessuna regressione).
