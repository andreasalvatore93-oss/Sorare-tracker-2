"""
Live prediction log (26/07) -- monitoraggio MAE live per i 4 ruoli Primeira Liga (Portogallo).
...
su dati storici in cache (formazione_portogallo/diagnostics/validate_*.py). Non
c'e' nessun tracciamento di quanto le predizioni REALI di produzione (quelle
scritte nei file .txt in portogallo_<ruolo>_all/) si rivelino accurate una volta
...
formazione_portogallo/diagnostics/resolve_live_predictions.py si occupa in seguito
...
Uso (da ciascuno dei 4 script predict, SOLO se non CALIBRATION_MODE):
    ...
    from formazione_portogallo.predict.live_prediction_log import log_live_prediction
