"""
Tool_formazione_mls_fwd_all (test TUTTI gli attaccanti Primeira Liga (Portogallo) in_season posseduti)

Estende test_multi_fwd.py: invece di una lista statica di 7 giocatori, legge
la lista COMPLETA degli attaccanti Primeira Liga (Portogallo) in_season posseduti (numero variabile,
generata da portogallo_fwd_discovery.py) da portogallo_fwd_discovery/player_slugs.json.
...
- Lista giocatori letta dinamicamente da portogallo_fwd_discovery/player_slugs.json
...
# formazione_portogallo/predict/test_mls_fwd_all.py`, quindi l'import diretto
...
# GLOBALE (tutti gli attaccanti Primeira Liga (Portogallo) di qualita', non solo posseduti) invece
...
DISCOVERY_FILE = os.path.join(
    'formazione_portogallo/output/portogallo_fwd_discovery_global' if CALIBRATION_MODE else 'formazione_portogallo/output/portogallo_fwd_discovery',
    'player_slugs.json')

# Fallback statico SOLO se portogallo_fwd_discovery/player_slugs.json non esiste
...
OUTPUT_DIR = 'formazione_portogallo/output/portogallo_fwd_calibration' if CALIBRATION_MODE else 'formazione_portogallo/output/portogallo_fwd_all'
...
_CIRCUIT_BREAKER_PATH = '/tmp/sorare_cloudfront_block_portogallo_fwd.marker'
...
def rigorous_backtest_prod_fwd(scores, is_home_flags,
                               ...
                               opponent_team_slugs_hist=None, league='portogallo',
                               offensive_values=None,
...
def build_prediction(player_slug):
    ...
    _opp_lambda_mult = opponent_strength.opponent_lambda_multiplier(
        'portogallo', 'fwd', next_opponent_team_slug, _opp_cutoff)
    ...
    _fwd_offense_delta = opponent_strength.fwd_offense_granular_delta(
        'portogallo', next_opponent_team_slug, _opp_cutoff, _offensive_hist)
    grezzo_nuovo = (level_score_atteso + media_granulari_pesata * fattore_trend_granulare
                    + _fwd_offense_delta)
...
def main():
    ...
    log("Avvio test TUTTI attaccanti Primeira Liga (Portogallo) in_season Tool_formazione...")
