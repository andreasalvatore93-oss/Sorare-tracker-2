# =====================================================================================
# BOT TERZO -- unificazione Bot Profit + Bot Definitivo (03/08).
# Motore REATTIVO di Definitivo (esecuzione, query, firma: codice PROVATO, copiato
# verbatim) + GATE TEMPORALE di Profit innestato (asse 'sconto vs media recente +
# trend + guadagno assoluto', vedi motore_affare.py). Regola a due assi IN AND:
#   spread live 1o/2o (Definitivo) AND cheap-vs-storia-recente (Profit).
# Lista nera DEDICATA (sorare_lista_nera_terzo.txt). Diagnostica-first: acquisto/
# offerta reali OFF di default. I due bot originali NON vengono toccati.
# NOTA: file generato da bots/bot_definitivo.py + trasformazioni; unica modifica di
# LOGICA e' il gate temporale in evaluate_event (vedi marcatori '# [TERZO]').
# =====================================================================================
...
# --- TERZO BOT: motore affare (gate temporale, asse Profit) ---
import sys as _sys
...
LISTA_NERA_PATH = os.environ.get('LISTA_NERA_PATH', 'sorare_lista_nera_terzo.txt')
...
def _medie_temporali_da_nodi(nodes, is_in_season, league_slug, eth_rate):
    """[TERZO] Dai NODI transazione gia' scaricati (zero query extra) calcola:
      - media_full: media prezzi nell'intera finestra _LOOKBACK_DAYS (baseline dello
        SCONTO temporale, come nel backtest validato);
      - media_rec1d / media_old: media dell'ultimo giorno vs resto finestra (per il TREND).
    Filtro stagione per le leghe escluse, identico a get_last_transaction_prices.
    Ritorna (media_full, media_rec1d, media_old); valori None se dati insufficienti."""
    ...
    return media_full, media_rec1d, media_old
...
def evaluate_event(player_slug, player_name, price_eur, card_slug, eth_rate, league_slug=None,
    ...
    # [TERZO] un solo fetch transazioni, riusato per liquidita', ultimo/penultimo prezzo
    # E medie temporali (asse Profit) -- zero query extra rispetto a Definitivo.
    _tx_nodes = _fetch_paginated_transaction_nodes(player_slug)
    count_7d, count_30d = _count_transactions_from_nodes(
        _tx_nodes, season_filter=is_in_season, player_slug=player_slug)
    ultimo_prezzo_transazione, penultimo_prezzo_transazione, terzo_prezzo_transazione = \
        get_last_transaction_prices(player_slug, is_in_season, league_slug, eth_rate, _tx_nodes)
    ...
    # [TERZO] GATE TEMPORALE (asse Profit) -- AND col segnale trasversale (spread live)
    # gia' verificato sopra. La carta deve essere davvero cheap vs la sua media recente,
    ...
    if not _gate['passa']:
        ...
        return False
    log(f"{player_name}: GATE TEMPORALE ok -- {_gate['motivo']}")
