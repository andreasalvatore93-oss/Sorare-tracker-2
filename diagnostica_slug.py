...
query CardBonusProbe($slug: String!) {
  anyCard(slug: $slug) {
    ...
    ... on Card {
      xp
      season { bonus percentage seasonBonus }
      collection { bonus percentage collectionBonus }
      experienceBonus
      powerBonus
      totalBonus
      bonusPercentage
...
def main():
    ...
    for slug in slugs:
        ...
        print(f"nome: {p.get('displayName')} | birthDay: {p.get('birthDay')} | u23Eligible: {p.get('u23Eligible')}")
