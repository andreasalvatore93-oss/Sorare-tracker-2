"""
aggregate_grid_search_gk.py

Aggrega i risultati del grid search (72 combinazioni) salvati da test_mid.py
per ogni portiere (mls_gk_all/grid_search/<slug>_grid.json), calcola
...
Output: mls_gk_all/grid_search/aggregato_<timestamp>.txt con la classifica
...
def log(msg):
    ...
    print(f"[{ts}] [aggregate_grid_search_gk] {msg}")
...
def main():
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("GRID SEARCH CROSS-PLAYER PORTIERI — CLASSIFICA COMBINAZIONI")
