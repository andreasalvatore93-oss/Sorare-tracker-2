"""
J1 100 Year Vision League (Giappone) Defender Discovery
...
lega J1 100 Year Vision League (Giappone) (slug 'j1-100-year-vision-league').
...
mlspa). Non conosciamo il codice equivalente per Giappone (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'j1-100-year-vision-league'. Query un po' meno
...
Output: giappone100_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_giappone100/output/giappone100_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO J1 100 Year Vision League (Giappone).
TARGET_LEAGUE_SLUG = 'j1-100-year-vision-league'
...
def log(msg):
    ...
    print(f"[{ts}] [giappone100_def_discovery] {msg}")
...
def discover_giappone100_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    J1 100 Year Vision League (Giappone)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Giappone
    dopo il fetch."""
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Giappone applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Giappone unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Giappone potrebbero essere
...
def main():
    log("Avvio discovery difensori J1 100 Year Vision League (Giappone) (in_season + classic)...")
    slugs, card_counts = discover_giappone100_defenders_all()
