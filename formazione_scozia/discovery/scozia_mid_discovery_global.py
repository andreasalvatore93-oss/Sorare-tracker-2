"""
scozia_mid_discovery_global.py

Discovery GLOBALE di tutti i centrocampisti Scottish Premiership in_season attivi (NON solo
...
resta sui soli giocatori posseduti via scozia_mid_discovery.py).
...
Output: scozia_mid_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_scozia/output/scozia_mid_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [scozia_mid_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE centrocampisti Scottish Premiership su {len(SCOZIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale centrocampisti Scottish Premiership unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale centrocampisti Scottish Premiership dopo filtro qualita': {len(slugs)}")
