"""
scozia_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti Scottish Premiership in_season attivi (NON solo
...
resta sui soli giocatori posseduti via scozia_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 12
squadre Scottish Premiership interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "premiership-gb-sct") { clubs(first: 50) { nodes {
slug name } } } }` -- 12/12 squadre restituite (workflow
verify_clubs.yml, run 30615960122).
...
Output: scozia_fwd_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_scozia/output/scozia_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "premiership-gb-sct")
# .clubs, 31/07) -- 12/12
SCOZIA_TEAM_SLUGS = [
    'aberdeen-aberdeen',
    'celtic-glasgow',
...
def log(msg):
    ...
    print(f"[{ts}] [scozia_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti Scottish Premiership su {len(SCOZIA_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(SCOZIA_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(SCOZIA_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti Scottish Premiership unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti Scottish Premiership dopo filtro qualita': {len(slugs)}")
