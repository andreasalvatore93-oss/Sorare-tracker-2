"""
Jupiler Pro League (Belgio) Defender Discovery
...
lega Jupiler Pro League (Belgio) (slug 'jupiler-pro-league').
...
mlspa). Non conosciamo il codice equivalente per Belgio (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'jupiler-pro-league'. Query un po' meno
...
Output: belgio_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_belgio/output/belgio_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Jupiler Pro League (Belgio).
TARGET_LEAGUE_SLUG = 'jupiler-pro-league'
...
def log(msg):
    ...
    print(f"[{ts}] [belgio_def_discovery] {msg}")
...
def discover_belgio_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Jupiler Pro League (Belgio)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Belgio
    dopo il fetch."""
    ...
    query = """
    query BelgiumDefenderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="BelgiumDefenderDiscovery")
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Belgio applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Belgio unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Belgio potrebbero essere
...
def main():
    log("Avvio discovery difensori Jupiler Pro League (Belgio) (in_season + classic)...")
    slugs, card_counts = discover_belgio_defenders_all()
