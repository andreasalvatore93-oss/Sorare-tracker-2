"""
Jupiler Pro League (Belgio) Goalkeeper Discovery
...
Output: belgio_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_belgio/output/belgio_gk_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [belgio_gk_discovery] {msg}")
...
def discover_belgio_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Belgio
    dopo il fetch."""
    ...
    query = """
    query BelgiumGoalkeeperDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="BelgiumGoalkeeperDiscovery")
...
def main():
    log("Avvio discovery portieri Jupiler Pro League (Belgio) (in_season + classic)...")
    slugs, card_counts = discover_belgio_goalkeepers_all()
