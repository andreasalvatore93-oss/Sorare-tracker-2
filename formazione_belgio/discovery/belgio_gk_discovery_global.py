"""
belgio_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri First Division A in_season attivi (NON solo
...
resta sui soli giocatori posseduti via belgio_gk_discovery.py).
...
Output: belgio_gk_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_belgio/output/belgio_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [belgio_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri First Division A su {len(BELGIO_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri First Division A unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri First Division A dopo filtro qualita': {len(slugs)}")
