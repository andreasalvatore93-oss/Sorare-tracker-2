"""
Jupiler Pro League (Belgio) Midfielder Discovery
...
Output: belgio_mid_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_belgio/output/belgio_mid_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [belgio_mid_discovery] {msg}")
...
def discover_belgio_midfielders_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Midfielder posseduti (qualunque lega) e scarta quelli non Belgio
    dopo il fetch."""
    ...
    query = """
    query BelgiumMidfielderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="BelgiumMidfielderDiscovery")
...
def main():
    log("Avvio discovery centrocampisti Jupiler Pro League (Belgio) (in_season + classic)...")
    slugs, card_counts = discover_belgio_midfielders_all()
