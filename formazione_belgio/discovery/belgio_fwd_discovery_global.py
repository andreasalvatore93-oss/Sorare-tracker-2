"""
belgio_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti First Division A in_season attivi (NON solo
...
resta sui soli giocatori posseduti via belgio_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 18
squadre First Division A interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "jupiler-pro-league") { clubs(first: 50) { nodes {
slug name } } } }` -- 18/18 squadre restituite (verifica dal vivo 31/07).
...
Output: belgio_fwd_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_belgio/output/belgio_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "jupiler-pro-league")
# .clubs, 31/07) -- 18/18
BELGIO_TEAM_SLUGS = [
...
def log(msg):
    ...
    print(f"[{ts}] [belgio_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti First Division A su {len(BELGIO_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(BELGIO_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(BELGIO_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti First Division A unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti First Division A dopo filtro qualita': {len(slugs)}")
