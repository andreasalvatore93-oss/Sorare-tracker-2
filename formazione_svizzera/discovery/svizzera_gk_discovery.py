"""
Super League (Svizzera) Goalkeeper Discovery
...
Output: svizzera_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_svizzera/output/svizzera_gk_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [svizzera_gk_discovery] {msg}")
...
def discover_svizzera_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Svizzera
    dopo il fetch."""
...
def main():
    log("Avvio discovery portieri Super League (Svizzera) (in_season + classic)...")
    slugs, card_counts = discover_svizzera_goalkeepers_all()
