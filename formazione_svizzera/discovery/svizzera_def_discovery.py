"""
Super League (Svizzera) Defender Discovery
...
lega Super League (Svizzera) (slug 'super-league-ch').
...
mlspa). Non conosciamo il codice equivalente per Svizzera (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'super-league-ch'. Query un po' meno
...
Output: svizzera_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_svizzera/output/svizzera_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Super League (Svizzera).
TARGET_LEAGUE_SLUG = 'super-league-ch'
...
def log(msg):
    ...
    print(f"[{ts}] [svizzera_def_discovery] {msg}")
...
def discover_svizzera_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Super League (Svizzera)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Svizzera
    dopo il fetch."""
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Svizzera applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Svizzera unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Svizzera potrebbero essere
...
def main():
    log("Avvio discovery difensori Super League (Svizzera) (in_season + classic)...")
    slugs, card_counts = discover_svizzera_defenders_all()
