"""
Super League (Svizzera) Midfielder Discovery
...
Output: svizzera_mid_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_svizzera/output/svizzera_mid_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [svizzera_mid_discovery] {msg}")
...
def discover_svizzera_midfielders_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Midfielder posseduti (qualunque lega) e scarta quelli non Svizzera
    dopo il fetch."""
...
def main():
    log("Avvio discovery centrocampisti Super League (Svizzera) (in_season + classic)...")
    slugs, card_counts = discover_svizzera_midfielders_all()
