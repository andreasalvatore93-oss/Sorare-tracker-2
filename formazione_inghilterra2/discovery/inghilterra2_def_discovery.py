"""
Championship (Inghilterra) Defender Discovery
...
lega Championship (Inghilterra) (slug 'football-league-championship').
...
mlspa). Non conosciamo il codice equivalente per Inghilterra (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'football-league-championship'. Query un po' meno
...
Output: inghilterra2_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_inghilterra2/output/inghilterra2_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Championship (Inghilterra).
TARGET_LEAGUE_SLUG = 'football-league-championship'
...
def log(msg):
    ...
    print(f"[{ts}] [inghilterra2_def_discovery] {msg}")
...
def discover_inghilterra2_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Championship (Inghilterra)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Inghilterra
    dopo il fetch."""
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Inghilterra applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Inghilterra unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Inghilterra potrebbero essere
...
def main():
    log("Avvio discovery difensori Championship (Inghilterra) (in_season + classic)...")
    slugs, card_counts = discover_inghilterra2_defenders_all()
