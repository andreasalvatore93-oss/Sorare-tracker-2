"""
Championship (Inghilterra) Forward Discovery
...
Output: inghilterra2_fwd_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_inghilterra2/output/inghilterra2_fwd_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [inghilterra2_fwd_discovery] {msg}")
...
def discover_inghilterra2_forwards_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Forward posseduti (qualunque lega) e scarta quelli non Inghilterra
    dopo il fetch."""
...
def main():
    log("Avvio discovery attaccanti Championship (Inghilterra) (in_season + classic)...")
    slugs, card_counts = discover_inghilterra2_forwards_all()
