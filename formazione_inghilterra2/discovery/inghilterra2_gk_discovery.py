"""
Championship (Inghilterra) Goalkeeper Discovery
...
Output: inghilterra2_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_inghilterra2/output/inghilterra2_gk_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [inghilterra2_gk_discovery] {msg}")
...
def discover_inghilterra2_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Inghilterra
    dopo il fetch."""
...
def main():
    log("Avvio discovery portieri Championship (Inghilterra) (in_season + classic)...")
    slugs, card_counts = discover_inghilterra2_goalkeepers_all()
