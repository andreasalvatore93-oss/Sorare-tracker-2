...
score_atteso invariato). Vedi compute_score_atteso_mid:
  ...
  risultato = grezzo_corretto * fattore_casa_trasferta
              + Stadio D (delta venue+avversario su offensivo/passaggio/gol subiti)
...
# formazione_inghilterra2/predict/test_mid.py`, quindi l'import diretto funziona a
...
# GLOBALE (tutti i centrocampisti MLS di qualita', non solo posseduti) invece
...
DISCOVERY_FILE = os.path.join(
    'formazione_inghilterra2/output/inghilterra2_mid_discovery_global' if CALIBRATION_MODE else 'formazione_inghilterra2/output/inghilterra2_mid_discovery',
    'player_slugs.json')
...
SHRINK_K_OUTLIER_MID = 15.0  # AGGIORNATO (14/08/2026): 5.0 -> 15.0, il centrocampo era l'unico ruolo rimasto al valore del 29/07 (DEF e FWD stavano gia' a 15, GK a 30). Rimisurato COL VOTO ACCESO, cioe' sull'atteso combinato che entra davvero nel knapsack da quando G fisso e' in produzione (13/08): il metro senza voto non vale piu' -- vedi la regola in CLAUDE.md. Confronto appaiato su 35.458 righe con voto, 1.496 giocatori: MAE 12.761 -> 12.645 (-0.116), meglio nel 100% dei ricampionamenti sui giocatori E in entrambe le meta' di calendario (-0.127 prima del 28/02, -0.106 dopo). Correlazione -0.001 e lift +0.55 entrambi INDISTINGUIBILI da zero (IC che contengono lo zero): nessun costo misurabile sull'ordinamento ne' sulla scelta delle cinque carte. Si applica perche' nessuna delle tre misure peggiora in modo distinguibile e una migliora in modo certo. Effetto collaterale voluto: la sovrastima di chi arriva da un periodo alto (residuo della fascia 65-75 meno quello della fascia 35-45) passa da -9.3 a -2.4. Dati e metodo in HANDOFF_UNIFICATO §8sexdecies, script taratura_striscia_oro.py.  # Storia: AGGIORNATO (29/07, modello unico GLOBALE su 25 leghe pooled): backtest walk-forward su ~2500 punti di test conferma guadagno pulito su entrambi i segmenti (-1.38% tot, -1.74%/-1.29%) -- il vecchio motivo dello scarto ("guadagno solo su n>=8") non regge piu' con questo volume di dati, stesso valore ora su TUTTE le leghe incluso MLS/Korea
...
OUTPUT_DIR = 'formazione_inghilterra2/output/inghilterra2_mid_calibration' if CALIBRATION_MODE else 'formazione_inghilterra2/output/inghilterra2_mid_all'
...
_CIRCUIT_BREAKER_PATH = '/tmp/sorare_cloudfront_block_inghilterra2_mid.marker'
...
def compute_score_atteso_mid(scores, is_home_flags, opponent_rankings,
                             ...
                             target_opponent_team_slug=None, target_cutoff_dt=None, league='inghilterra2',
...
def rigorous_backtest_prod_mid(scores, is_home_flags, opponent_rankings,
                               ...
                               presence_rate=None, league='inghilterra2', detail_ok_flags=None):
...
def build_prediction(player_slug):
    ...
    # coefficiente di sensibilita'): -9.20% rimuovendolo vs -5.60% con gol
    ...
    # vedi test_def.py per la spiegazione estesa. Rivalidato su 6 campionati: -0.45% MAE.
    # SOSTITUISCE il vecchio condizionamento venue di Stadio D sotto (che
    # correggeva la media grezza di level_score per casa/trasferta) -- il
    # tasso di eventi e' gia' una stima piu' informativa del level_score
    # atteso, ricondizionarlo per venue sarebbe un doppio conteggio dello
    # stesso segnale. Il condizionamento venue/avversario sulle sotto-categorie
    # granulari sotto (offensivo/passaggio/gol_subiti) resta invariato.
    # opponent_lambda_mult (29/07, vedi opponent_strength.py): gol subiti dal
    ...
    _opp_lambda_mult = opponent_strength.opponent_lambda_multiplier(
        'inghilterra2', 'mid', next_opponent_team_slug, datetime.datetime.utcnow())
    ...
    score_atteso = compute_score_atteso_mid(
        ...
        target_cutoff_dt=_opp_cutoff, league='inghilterra2', detail_ok_flags=detail_ok_flags)
    ...
    # Grid search di calibrazione GIA' concluso (19 centrocampisti posseduti,
    # MAE medio 15.62, copertura 68.3%). Non serve piu' rieseguire 72
    # combinazioni ad ogni giocatore in produzione — un solo backtest sui
    # parametri fissati, molto piu' veloce. NOTA: questi parametri sono
    # tarati sui 19 posseduti, non ancora sui 346 centrocampisti MLS totali
    ...
    if CALIBRATION_MODE:
        ...
        rigorous_bt['label'] = (f"hl={HALF_LIFE_GAMES}+range={RANGE_MULTIPLIER}x+"
                                f"opp_sens={OPPONENT_SENSITIVITY}+trend_int={TREND_INTENSITY} (FISSATA 19 posseduti)")
...
def main():
    ...
    for idx, slug in enumerate(slugs_to_process, 1):
        ...
        log_live_prediction(OUTPUT_DIR, CALIBRATION_MODE, 'mid', result)
