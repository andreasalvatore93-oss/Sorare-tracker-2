...
score_atteso invariato). Vedi compute_score_atteso_def:
  ...
  risultato = grezzo_corretto * fattore_casa_trasferta
              + Stadio D (delta venue+avversario su gol subiti/passaggio/CS)
  range_confidenza = +/- dev_std_pesata * RANGE_MULTIPLIER
L'avversario entra via opponent_lambda_multiplier (decisivi) e via Stadio D
(granulare, flag forte/debole), mai come fattore moltiplicativo finale.
...
1.0 -- da ricalibrare", stantio da mesi): HALF_LIFE_GAMES=30.0, RANGE_
...
# formazione_inghilterra2/predict/test_def.py`, quindi l'import diretto funziona a
...
DISCOVERY_FILE = os.path.join(
    'formazione_inghilterra2/output/inghilterra2_def_discovery_global' if CALIBRATION_MODE else 'formazione_inghilterra2/output/inghilterra2_def_discovery',
    'player_slugs.json')
...
# cioe' il condizionamento granulare (gol subiti / passaggio / clean sheet) su
# "avversario forte o debole" entrava nel punteggio dei difensori.
# PERCHE' SPENTA: peggiorava l'ORDINAMENTO dei difensori. Riverifica
# indipendente del 04/08 (arm 4), n=25.738 righe e 263 giornate, confermata su
# GitHub Actions e in locale con numeri identici a 6 decimali dopo la messa in
# ordine dei glob: correlazione +0.005/+0.006 (IC 95% bootstrap sulle giornate
# [+0.00186, +0.01055], tutto sopra zero), MAE -0.016/-0.020, lift +1.09/+1.27.
# Segno identico in tutti e tre gli ambienti misurati.
# Cosa NON si compra: MAE e lift non sono dimostrati dal bootstrap (i loro IC
# comprendono lo zero). Si compra un miglioramento piccolo e reale di
# ordinamento, niente di piu'.
# SOLO i difensori: sui centrocampisti lo stesso intervento e' misurato nullo
# (MID n=24.478, tutte e tre le differenze dentro il rumore).
...
# NB: NON e' use_stadio_d=False, che spegne anche il condizionamento
# casa/trasferta. Quello resta acceso.
STADIO_D_AVVERSARIO = False
...
# (formazione_inghilterra2/diagnostics/validate_outlier_shrinkage.py) mostra che tirare
...
MEDIA_RUOLO_DEF_PRIOR = 51.34  # SOLO DIAGNOSTICO (marcato P7/passaggio 2): la produzione usa il prior DINAMICO da presence_rate (vedi compute_score_atteso_def, media_ruolo_prior = 45.36+7.96*presence_rate), non questa costante statica.
...
OUTPUT_DIR = 'formazione_inghilterra2/output/inghilterra2_def_calibration' if CALIBRATION_MODE else 'formazione_inghilterra2/output/inghilterra2_def_all'
...
_CIRCUIT_BREAKER_PATH = '/tmp/sorare_cloudfront_block_inghilterra2_def.marker'
...
# Logica identica a formazione_inghilterra2/diagnostics/validate_level_score_event_rate.py,
...
def compute_score_atteso_def(scores, is_home_flags, opponent_rankings,
                             ...
                             next_opponent_team_slug=None, next_game_date=None, league='inghilterra2',
    ...
    usava una formula moltiplicativa vecchia, diversa da quella reale del consiglio).

    Replica ESATTAMENTE le righe storiche di build_prediction (level_score atteso da
    tassi di eventi + granulari additivi con trend + shrinkage outlier + fattore
    casa/trasferta sul residuo + Stadio D: correzioni condizionate venue+avversario
    su gol_subiti/passaggio/clean_sheet). Chiamata coi default = risultato identico
...
def rigorous_backtest_prod_def(scores, is_home_flags, opponent_rankings,
                               ...
                               opponent_team_slugs_hist=None, game_dates_hist=None, league='inghilterra2',
    ...
    vecchio rigorous_backtest, che usava una formula moltiplicativa diversa dalla
    produzione (fattore_forza_avversario + granulare moltiplicativo) e quindi calibrava
    ...
    n = len(scores)
    for i in range(min_history, n):
        predetto = compute_score_atteso_def(
...
def build_prediction(player_slug):
    ...
    if CALIBRATION_MODE and not future_games:
        ...
        grid_results = run_grid_search_prod_def(
            ...
            min_history=6,
            opponent_team_slugs_hist=opponent_team_slugs_hist,
            game_dates_hist=game_dates_hist, league='inghilterra2',
            presence_rate=presence_rate, detail_ok_flags=detail_ok_flags)
    ...
    fattore_clean_sheet = compute_split_factor(clean_sheet_values, is_home_flags, next_is_home, weights_det)
    ...
    _opp_lambda_mult = opponent_strength.opponent_lambda_multiplier(
        'inghilterra2', 'def', next_opponent_team_slug, _opp_cutoff)
    ...
    # formazione_inghilterra2/diagnostics/selection_quality.py (lift catturato fra
    ...
    score_ordinamento = compute_score_atteso_def(
        ...
        league='inghilterra2', detail_ok_flags=detail_ok_flags)
