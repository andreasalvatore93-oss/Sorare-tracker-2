"""
Resolve Live Predictions (26/07) -- monitoraggio MAE live per i 4 ruoli K League.
...
che le modifiche al modello reggano anche live, sia per scoprire in futuro
un eventuale drift del modello.

Ogni predizione generata in produzione (formazione_kleague/predict/test_gk.py,
test_def.py, test_mid.py, test_mls_fwd_all.py -- MAI in CALIBRATION_MODE)
viene loggata come entry "pending" in
formazione_kleague/output/kleague_<ruolo>_all/prediction_log.json (vedi
formazione_kleague/predict/live_prediction_log.py). Questo script:

1. Legge le entry pending di ciascun ruolo.
2. Per ognuna, controlla la cache di produzione gia' presente
   (kleague_<ruolo>_all/.cache/<slug>_detail_cache.json) per vedere se ora
...
Uso:
  python formazione_kleague/diagnostics/resolve_live_predictions.py
  python formazione_kleague/diagnostics/resolve_live_predictions.py --report-only
  python formazione_kleague/diagnostics/resolve_live_predictions.py --last-n 20
"""
import os
import sys
import json
