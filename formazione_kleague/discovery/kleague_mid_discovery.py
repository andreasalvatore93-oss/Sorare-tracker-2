...
Output: kleague_mid_discovery/player_slugs.json -- lista JSON di slug univoci,
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_kleague/output/kleague_mid_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [kleague_mid_discovery] {msg}")
...
def discover_kleague_midfielders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Midfielder, lega K
    ...
    server-side, quindi la query recupera TUTTI i Midfielder posseduti
    ...
    query = """
    query KLeagueMidfielderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="KLeagueMidfielderDiscovery")
...
def main():
    log("Avvio discovery centrocampisti K League 1 (in_season + classic)...")
    slugs, card_counts = discover_kleague_midfielders_all()
