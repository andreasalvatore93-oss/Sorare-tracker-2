"""
kleague_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri K League 1 in_season attivi (NON solo
...
resta sui soli giocatori posseduti via kleague_gk_discovery.py).

Clone di mls_gk_discovery_global.py, stesso approccio: per ognuna delle 12
squadre K League 1 interroga Club(slug).anyPlayers per il roster completo,
...
Output: kleague_gk_discovery_global/player_slugs.json -- lista JSON di slug
giocatore portiere unici, deduplicati su tutte le 12 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_kleague/output/kleague_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [kleague_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri K League 1 su {len(KLEAGUE_TEAM_SLUGS)} squadre...")
