...
Output: kleague_fwd_discovery/player_slugs.json -- lista JSON di slug univoci,
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_kleague/output/kleague_fwd_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [kleague_fwd_discovery] {msg}")
...
def discover_kleague_forwards_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Forward, lega K
    ...
    server-side, quindi la query recupera TUTTI i Forward posseduti
    ...
    query = """
    query KLeagueForwardDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="KLeagueForwardDiscovery")

            search = ((data.get('data') or {}).get('user') or {}).get('searchCards') or {}
            hits = search.get('hits') or []
            if page == 1:
                log(f"Totale carte Forward/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
...
def main():
    log("Avvio discovery attaccanti K League 1 (in_season + classic)...")
    slugs, card_counts = discover_kleague_forwards_all()
