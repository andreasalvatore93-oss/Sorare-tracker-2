"""
kleague_mid_discovery_global.py

Discovery GLOBALE di tutti i centrocampisti K League 1 in_season attivi
(NON solo quelli posseduti dall'utente) -- usata per allargare il campione
del grid search di calibrazione, non per l'uso finale (schierare la
formazione, che resta sui soli giocatori posseduti via
kleague_mid_discovery.py).

Clone di kleague_gk_discovery_global.py, stesso approccio: per ognuna delle
12 squadre K League 1 interroga Club(slug).anyPlayers per il roster
completo, poi filtra lato client per posizione Midfielder (anyPositions del
giocatore). Nessuno scope utente richiesto: query pubblica.

Slug squadre ottenuti dal vivo (26/07) con la query pubblica
`football { competition(slug: "k-league-1") { clubs(first: 50) { nodes {
slug name } } } }` -- 12/12 squadre restituite (coerente con il numero di
squadre della K League 1).

Filtro qualita' (26/07, stesso principio di MLS 25/07): tenuti solo i
...
la calibrazione del modello. NOTA: i punteggi K League sono strutturalmente
piu' bassi di quelli MLS (vedi note in kleague_gk_discovery.py), quindi
questa soglia -- ereditata identica da MLS -- andra' rivalutata quando ci
sara' storico reale K League da osservare.

Output: kleague_mid_discovery_global/player_slugs.json -- lista JSON di
slug giocatore centrocampista unici, deduplicati su tutte le 12 squadre,
FILTRATI per qualita'.
"""
import os
import json
import time
...
OUTPUT_DIR = 'formazione_kleague/output/kleague_mid_discovery_global'
TARGET_POSITION = 'Midfielder'

# Slug ufficiali confermati dal vivo (query Competition(slug: "k-league-1")
# .clubs, 26/07) -- 12/12
KLEAGUE_TEAM_SLUGS = [
    'anyang-anyang',
...
def log(msg):
    ...
    print(f"[{ts}] [kleague_mid_discovery_global] {msg}")
...
# FILTRO QUALITA' (26/07, stesso principio applicato a MLS il 25/07): la
# discovery globale serve a calibrare il modello sui giocatori che l'utente
# comprerebbe davvero, non su tutti i tesserati di un ruolo -- i giocatori
# "scarsi" (media bassa) non li comprerebbe comunque e inquinerebbero la
...
def main():
    log(f"Avvio discovery GLOBALE centrocampisti K League 1 su {len(KLEAGUE_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    for idx, team_slug in enumerate(KLEAGUE_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(KLEAGUE_TEAM_SLUGS)}] Squadra: {team_slug}")
        players = fetch_team_players_by_position(team_slug, TARGET_POSITION)
        all_slugs.update(players)
        time.sleep(0.3)

    slugs = sorted(all_slugs)
    log(f"Totale centrocampisti K League 1 unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale centrocampisti K League 1 dopo filtro qualita': {len(slugs)}")
