"""
kleague_def_discovery_global.py

Discovery GLOBALE di tutti i difensori K League 1 in_season attivi (NON solo
...
resta sui soli giocatori posseduti via kleague_def_discovery.py).
...
completo, poi filtra lato client per posizione Defender (anyPositions del
...
Output: kleague_def_discovery_global/player_slugs.json -- lista JSON di
slug giocatore difensore unici, deduplicati su tutte le 12 squadre,
...
OUTPUT_DIR = 'formazione_kleague/output/kleague_def_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [kleague_def_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE difensori K League 1 su {len(KLEAGUE_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale difensori K League 1 unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale difensori K League 1 dopo filtro qualita': {len(slugs)}")
