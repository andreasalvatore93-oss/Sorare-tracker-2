"""
K League Defender Discovery

Scopre TUTTI gli slug giocatore (deduplicati) delle carte possedute da
'crowss' che sono: rarity=limited, posizione=Defender, in_season_eligible=true,
lega K League 1 (slug 'k-league-1', confermato attivo/reale -- gia' usato in
EXCLUDED_LEAGUE_SLUGS in bots/bot_definitivo.py e affini).
...
mlspa). Non conosciamo il codice equivalente per K League (rischio di
indovinare male e ottenere risultati sbagliati/silenziosamente vuoti), quindi
qui il filtro lega e' fatto lato CLIENT: la query recupera anche
...
si tiene solo chi ha domesticLeague.slug == 'k-league-1'. Query un po' meno
...
Output: kleague_def_discovery/player_slugs.json -- lista JSON di slug univoci,
usata dal job 'discover' nel workflow per generare dinamicamente la matrix
del job 'predict'.
"""
import os
import json
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_kleague/output/kleague_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO K League 1 (deciso con l'utente 26/07, K League 2
# esplicitamente esclusa).
TARGET_LEAGUE_SLUG = 'k-league-1'
...
def log(msg):
    ...
    print(f"[{ts}] [kleague_def_discovery] {msg}")
...
def discover_kleague_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega K
    League 1) posseduti da user_slug, SIA in_season CHE classic. Filtro lega
    lato CLIENT (vedi docstring modulo) -- nessun filtro active_competitions
    server-side, quindi la query recupera TUTTI i Defender posseduti
    (qualunque lega) e scarta quelli non K League dopo il fetch."""
    ...
    query = """
    query KLeagueDefenderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        refinements = [
            {"field": "position", "operator": "EQUAL", "values": [{"stringValue": "Defender"}]},
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="KLeagueDefenderDiscovery")

            search = ((data.get('data') or {}).get('user') or {}).get('searchCards') or {}
            hits = search.get('hits') or []
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro K League applicato dopo il fetch")
            if not hits:
                break
...
# NOTA (26/07, avviso esplicito dell'utente): i punteggi K League sono
# strutturalmente PIU' BASSI di quelli MLS -- le soglie MIN_ACTIVITY_SCORE
# (0.0) e MIN_L10_SCORE (35.0) sono riusate identiche come punto di partenza
# ma potrebbero tagliare troppo/troppo poco per questo campionato. Da
# rivalutare quando ci sara' storico reale K League da osservare (stesso
...
def main():
    log("Avvio discovery difensori K League 1 (in_season + classic)...")
    slugs, card_counts = discover_kleague_defenders_all()
