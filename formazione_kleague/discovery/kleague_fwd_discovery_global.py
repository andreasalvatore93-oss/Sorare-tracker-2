"""
kleague_fwd_discovery_global.py

Discovery GLOBALE di tutti gli attaccanti K League 1 in_season attivi (NON
solo quelli posseduti dall'utente) -- usata per allargare il campione del
grid search di calibrazione, non per l'uso finale (schierare la
formazione, che resta sui soli giocatori posseduti via
kleague_fwd_discovery.py).
...
completo, poi filtra lato client per posizione Forward (anyPositions del
...
Output: kleague_fwd_discovery_global/player_slugs.json -- lista JSON di
slug giocatore attaccante unici, deduplicati su tutte le 12 squadre,
...
OUTPUT_DIR = 'formazione_kleague/output/kleague_fwd_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [kleague_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti K League 1 su {len(KLEAGUE_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale attaccanti K League 1 unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti K League 1 dopo filtro qualita': {len(slugs)}")
