...
- Lista giocatori letta dinamicamente da kleague_fwd_discovery/player_slugs.json
...
DISCOVERY_FILE = os.path.join(
    'formazione_kleague/output/kleague_fwd_discovery_global' if _USE_GLOBAL_POOL else 'formazione_kleague/output/kleague_fwd_discovery',
    'player_slugs.json')

# Fallback statico SOLO se kleague_fwd_discovery/player_slugs.json non esiste
...
OUTPUT_DIR = 'formazione_kleague/output/kleague_fwd_calibration' if CALIBRATION_MODE else 'formazione_kleague/output/kleague_fwd_all'
...
_CIRCUIT_BREAKER_PATH = '/tmp/sorare_cloudfront_block_kleague_fwd.marker'
...
def rigorous_backtest_prod_fwd(scores, is_home_flags,
                               ...
                               opponent_team_slugs_hist=None, league='kleague',
                               offensive_values=None,
...
def build_prediction(player_slug):
    ...
    # opponent_lambda_mult (29/07, vedi opponent_strength.py): gol subiti dal prossimo avversario nelle ultime 10 partite (dato storico reale, non il domesticLeagueRanking contaminato). Validato: -0.58% MAE.
    _opp_lambda_mult = opponent_strength.opponent_lambda_multiplier(
        'kleague', 'fwd', next_opponent_team_slug, _opp_cutoff)
    ...
    _fwd_offense_delta = opponent_strength.fwd_offense_granular_delta(
        'kleague', next_opponent_team_slug, _opp_cutoff, _offensive_hist)
    grezzo_nuovo = (level_score_atteso + media_granulari_pesata * fattore_trend_granulare
                    + _fwd_offense_delta)
