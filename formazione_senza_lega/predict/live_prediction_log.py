"""
Live prediction log (26/07) -- monitoraggio MAE live per i 4 ruoli Senza campionato.
...
su dati storici in cache (formazione_senza_lega/diagnostics/validate_*.py). Non
c'e' nessun tracciamento di quanto le predizioni REALI di produzione (quelle
scritte nei file .txt in senza_lega_<ruolo>_all/) si rivelino accurate una volta
...
formazione_senza_lega/diagnostics/resolve_live_predictions.py si occupa in seguito
...
Uso (da ciascuno dei 4 script predict, SOLO se non CALIBRATION_MODE):
    ...
    from formazione_senza_lega.predict.live_prediction_log import log_live_prediction
