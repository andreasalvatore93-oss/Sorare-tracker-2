"""
Senza campionato Goalkeeper Discovery
...
League (vedi kleague_gk_discovery.py): la query recupera anche
...
Output: senza_lega_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_senza_lega/output/senza_lega_gk_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [senza_lega_gk_discovery] {msg}")
...
def discover_senza_lega_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Goalkeeper, lega
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Senza campionato
    dopo il fetch."""
    ...
    query = """
    query TurkeyGoalkeeperDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="TurkeyGoalkeeperDiscovery")
...
def main():
    log("Avvio discovery portieri Senza campionato (in_season + classic)...")
    slugs, card_counts = discover_senza_lega_goalkeepers_all()
