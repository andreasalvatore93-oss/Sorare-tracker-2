"""
Senza campionato Defender Discovery
...
lega Senza campionato (slug 'spor-toto-super-lig').
...
mlspa). Non conosciamo il codice equivalente per Senza campionato (rischio di
...
League (vedi kleague_def_discovery.py): la query recupera anche
...
si tiene solo chi ha domesticLeague.slug == 'spor-toto-super-lig'. Query un po' meno
...
Output: senza_lega_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_senza_lega/output/senza_lega_def_discovery'
...
# Lega target -- SOLO Senza campionato.
TARGET_LEAGUE_SLUG = None  # nessuna lega: vedi il filtro invertito piu' sotto
...
def log(msg):
    ...
    print(f"[{ts}] [senza_lega_def_discovery] {msg}")
...
def discover_senza_lega_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Senza campionato) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Senza campionato
    dopo il fetch."""
    ...
    query = """
    query TurkeyDefenderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="TurkeyDefenderDiscovery")
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Senza campionato applicato dopo il fetch")
            if not hits:
                break
            ...
            for h in hits:
                ...
                # Filtro INVERTITO (27/07): qui si tengono SOLO le carte il cui
                # activeClub non ha domesticLeague (giocatori in competizioni
                # tipo coppe europee, o club fuori dai campionati mappati da
                # Sorare). Sono ~64 carte che nessuna pipeline per campionato
                # poteva vedere -- sparivano del tutto dal generatore.
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Senza campionato unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Senza campionato potrebbero essere
...
def main():
    log("Avvio discovery difensori Senza campionato (in_season + classic)...")
    slugs, card_counts = discover_senza_lega_defenders_all()
