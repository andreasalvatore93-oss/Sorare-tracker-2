"""
Senza campionato Midfielder Discovery
...
Output: senza_lega_mid_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_senza_lega/output/senza_lega_mid_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [senza_lega_mid_discovery] {msg}")
...
def discover_senza_lega_midfielders_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Midfielder posseduti (qualunque lega) e scarta quelli non Senza campionato
    dopo il fetch."""
...
def main():
    log("Avvio discovery centrocampisti Senza campionato (in_season + classic)...")
    slugs, card_counts = discover_senza_lega_midfielders_all()
