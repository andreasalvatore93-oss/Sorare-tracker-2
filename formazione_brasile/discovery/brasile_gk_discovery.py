"""
Brasileirao Goalkeeper Discovery
...
Output: brasile_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_brasile/output/brasile_gk_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [brasile_gk_discovery] {msg}")
...
def discover_brasile_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
...
def main():
    log("Avvio discovery portieri Brasileirao (in_season + classic)...")
    slugs, card_counts = discover_brasile_goalkeepers_all()
