"""
Super League (Grecia) Forward Discovery
...
Output: grecia_fwd_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_grecia/output/grecia_fwd_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [grecia_fwd_discovery] {msg}")
...
def discover_grecia_forwards_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Forward posseduti (qualunque lega) e scarta quelli non Grecia
    dopo il fetch."""
...
def main():
    log("Avvio discovery attaccanti Super League (Grecia) (in_season + classic)...")
    slugs, card_counts = discover_grecia_forwards_all()
