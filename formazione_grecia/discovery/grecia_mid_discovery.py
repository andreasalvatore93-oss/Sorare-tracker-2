"""
Super League (Grecia) Midfielder Discovery
...
Output: grecia_mid_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_grecia/output/grecia_mid_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [grecia_mid_discovery] {msg}")
...
def discover_grecia_midfielders_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Midfielder posseduti (qualunque lega) e scarta quelli non Grecia
    dopo il fetch."""
...
def main():
    log("Avvio discovery centrocampisti Super League (Grecia) (in_season + classic)...")
    slugs, card_counts = discover_grecia_midfielders_all()
