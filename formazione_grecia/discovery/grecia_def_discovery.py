"""
Super League (Grecia) Defender Discovery
...
lega Super League (Grecia) (slug 'super-league-1').
...
mlspa). Non conosciamo il codice equivalente per Grecia (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'super-league-1'. Query un po' meno
...
Output: grecia_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_grecia/output/grecia_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Super League (Grecia).
TARGET_LEAGUE_SLUG = 'super-league-1'
...
def log(msg):
    ...
    print(f"[{ts}] [grecia_def_discovery] {msg}")
...
def discover_grecia_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Super League (Grecia)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Grecia
    dopo il fetch."""
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Grecia applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Grecia unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Grecia potrebbero essere
...
def main():
    log("Avvio discovery difensori Super League (Grecia) (in_season + classic)...")
    slugs, card_counts = discover_grecia_defenders_all()
