...
TREND_INTENSITY = 1.0  # FISSATO (25/07): grid search cross-player su 19 centrocampisti POSSEDUTI (non ancora tutti i 346 MLS) — combinazione vincente hl=12.0/range=1.4/opp_sens=29.0/trend_int=1.0, MAE medio 15.62, copertura 68.3%. Da ricalibrare quando si allargherà il grid search a tutti i 346 centrocampisti MLS (discovery globale già pronta).
MIN_MINUTES_PLAYED = 60  # partite giocate sotto questa soglia (subentri) escluse dalla finestra
MIN_STARTER_ODDS = 0.70  # RIATTIVATO (25/07) per l'USO REALE (schierare formazione): grid search di calibrazione ormai concluso sui 19 posseduti, il filtro va tenuto attivo in produzione per escludere chi probabilmente non gioca. Se si rifà grid search (es. sui 346 MLS totali), riportare a 0.0 temporaneamente.
