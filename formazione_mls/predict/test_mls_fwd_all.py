...
# formazione_mls/predict/test_mls_fwd_all.py`, quindi l'import diretto
...
SHRINK_K_OUTLIER_FWD = 5.0  # AGGIORNATO (14/08/2026): 15.0 -> 5.0, si torna al valore pre-03/08. MOTIVO: il 15.0 fu scelto col metro SENZA VOTO, quando G fisso non esisteva (e' del 13/08) -- tarato su un modello che non esiste piu', vedi la regola in CLAUDE.md. Rimisurato col voto acceso, confronto appaiato su 30.311 righe, 1.500+ giocatori, taglio nei due periodi di calendario: il MAE cala di 0.148 (meglio nel 100% dei ricampionamenti) e soprattutto il LIFT sale di 1.60 con IC90 [+0.49;+2.75] che ESCLUDE lo zero -- nel periodo recente +1.91, IC [+0.77;+3.12]. La correlazione peggiora di 0.0039 (IC che esclude lo zero anch'esso): e' un conflitto vero, non un'astensione. Applicato per DECISIONE ESPLICITA DELL'UTENTE (14/08) dopo aver visto il bilanciamento: 'preferisco centrare i primi della classe e piuttosto sbagliare gli altri'. Il lift misura la scelta delle cinque carte da schierare (la testa della classifica), la correlazione l'ordine di TUTTA la lista, compresi i giocatori che non si schierano mai: il costo cade sui consigli di acquisto dello scouting, il guadagno sulle formazioni. Vedi la sezione 'i tre cancelli si PESANO' in CLAUDE.md. Coerente con la diagnosi indipendente dello stesso giorno: gli attaccanti che vengono da un periodo eccezionale (media >75) erano SOTTOSTIMATI di 8.6 punti (n=254, IC [+2.96;+8.26]) -- comprimere meno li riporta su. Dati in HANDOFF_UNIFICATO §8sexdecies, script taratura_striscia_oro.py.  # Storia: AGGIORNATO (03/08): 5.0 -> 15.0. Rimisurato DOPO il fix delle partite senza dettaglio, ed e' proprio quel fix a spostare l'ottimo: prima il grezzo era gonfiato (il punteggio intero finiva nel granulare), quindi tirarlo verso il prior sembrava dannoso e la taratura sceglieva poco shrinkage. Col grezzo corretto conviene tirare di piu'. Misurato su 18.992 punti walk-forward, 885 attaccanti: MAE 14.092 -> 14.049 (-0.31%), correlazione previsto/realizzato 0.250 -> 0.257, selezione dei primi cinque 28.2% -> 28.4%. Tutte e tre le misure si muovono nello stesso verso, che e' la condizione per applicare. Storia del valore precedente sotto.  # AGGIORNATO (29/07, modello unico GLOBALE su 25 leghe pooled): backtest walk-forward su ~1700 punti di test, MAE -0.49% -- la vecchia esclusione "peggiora fuori MLS" non e' risultata riproducibile col volume di dati attuale (vedi RIASSUNTO sez.29), stesso valore ora su TUTTE le leghe
...
_CIRCUIT_BREAKER_PATH = '/tmp/sorare_cloudfront_block_mls_fwd.marker'
...
def build_prediction(player_slug):
    ...
    _opp_lambda_mult = opponent_strength.opponent_lambda_multiplier(
        'mls', 'fwd', next_opponent_team_slug, _opp_cutoff)
    ...
    score_ordinamento = compute_score_atteso_fwd(
        ...
        league='mls', offensive_values=offensive_values,
        detail_ok_flags=detail_ok_flags)
