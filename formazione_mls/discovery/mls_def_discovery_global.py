"""
mls_def_discovery_global.py

Discovery GLOBALE di tutti i difensori MLS in_season attivi (NON solo
...
resta sui soli giocatori posseduti via mls_def_discovery.py).
...
filtra lato client per posizione Defender (anyPositions del giocatore).
...
Output: mls_def_discovery_global/player_slugs.json -- lista JSON di slug
giocatore difensore unici, deduplicati su tutte le 30 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_mls/output/mls_def_discovery_global'
...
def main():
    log(f"Avvio discovery GLOBALE difensori MLS su {len(MLS_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale difensori MLS unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale difensori MLS dopo filtro qualita': {len(slugs)}")
