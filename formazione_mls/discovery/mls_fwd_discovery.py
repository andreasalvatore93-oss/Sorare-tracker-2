...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_mls/output/mls_fwd_discovery'
...
def discover_mls_forwards_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Forward, lega MLS)
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Forward/{type_key}/MLS trovate: {search.get('nbHits')} "
