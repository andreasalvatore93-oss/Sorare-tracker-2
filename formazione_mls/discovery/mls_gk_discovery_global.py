"""
mls_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri MLS in_season attivi (NON solo
...
resta sui soli giocatori posseduti via mls_gk_discovery.py).
...
filtra lato client per posizione Goalkeeper (anyPositions del giocatore).
...
Output: mls_gk_discovery_global/player_slugs.json -- lista JSON di slug
giocatore portiere unici, deduplicati su tutte le 30 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_mls/output/mls_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [mls_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri MLS su {len(MLS_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri MLS unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri MLS dopo filtro qualita': {len(slugs)}")
