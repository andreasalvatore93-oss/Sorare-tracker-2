...
Output: mls_def_discovery/player_slugs.json -- lista JSON di slug univoci,
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_mls/output/mls_def_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [mls_def_discovery] {msg}")
...
def discover_mls_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega MLS)
    ...
    query = """
    query MlsDefenderDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="MlsDefenderDiscovery")

            search = ((data.get('data') or {}).get('user') or {}).get('searchCards') or {}
            hits = search.get('hits') or []
            if page == 1:
                log(f"Totale carte Defender/{type_key}/MLS trovate: {search.get('nbHits')} "
...
def main():
    log("Avvio discovery difensori MLS (in_season + classic)...")
    slugs, card_counts = discover_mls_defenders_all()
