...
_end = next(i for i, l in enumerate(_lines)
            if i > _start and l.strip() == 'score_atteso += p_gioca * delta_passaggio_venue')
_block = '\n'.join(l[4:] if l.startswith('    ') else l for l in _lines[_start:_end + 1])
assert 'score_atteso = p_gioca' in _block and 'score_atteso +=' in _block
...
def arrays_from_cache(path):
    ...
    d = {k: [] for k in 'scores is_home pas resid gran pos neg'.split()}
    for v in nodes:
        gs = v.get('score', 0.0)
...
def run_case(d, next_is_home, p_gioca):
    n = len(d['scores'])
