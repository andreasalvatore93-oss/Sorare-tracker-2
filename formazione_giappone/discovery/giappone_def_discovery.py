"""
J1 League (Giappone) Defender Discovery
...
lega J1 League (Giappone) (slug 'j1-league').
...
si tiene solo chi ha domesticLeague.slug == 'j1-league'. Query un po' meno
...
Output: giappone_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_giappone/output/giappone_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO J1 League (Giappone).
TARGET_LEAGUE_SLUG = 'j1-league'
...
def log(msg):
    ...
    print(f"[{ts}] [giappone_def_discovery] {msg}")
...
def discover_giappone_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    J1 League (Giappone)) posseduti da user_slug, SIA in_season CHE classic. Filtro
...
def main():
    log("Avvio discovery difensori J1 League (Giappone) (in_season + classic)...")
    slugs, card_counts = discover_giappone_defenders_all()
