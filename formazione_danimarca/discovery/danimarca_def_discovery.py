"""
Superliga (Danimarca) Defender Discovery
...
Output: danimarca_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_danimarca/output/danimarca_def_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [danimarca_def_discovery] {msg}")
...
def discover_danimarca_defenders_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Defender posseduti (qualunque lega) e scarta quelli non Danimarca
    dopo il fetch."""
...
def main():
    log("Avvio discovery difensori Superliga (Danimarca) (in_season + classic)...")
    slugs, card_counts = discover_danimarca_defenders_all()
