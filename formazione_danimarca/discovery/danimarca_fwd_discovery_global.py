"""
danimarca_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti Superliga (Danimarca) in_season attivi (NON solo
...
resta sui soli giocatori posseduti via danimarca_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 12
squadre Superliga (Danimarca) interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "superliga-dk") { clubs(first: 50) { nodes {
slug name } } } }` -- 12/12 squadre restituite (workflow
...
Output: danimarca_fwd_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_danimarca/output/danimarca_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "superliga-dk")
# .clubs, 31/07) -- 12/12
DANIMARCA_TEAM_SLUGS = [
    'agf-arhus',
    'brondby-brondby',
    'horsens-horsens',
    'kobenhavn-kobenhavn',
    'lyngby-lyngby-kongens-lyngby',
    'midtjylland-herning',
    'nordsjaelland-farum',
    'ob-odense',
    'randers-randers',
...
def log(msg):
    ...
    print(f"[{ts}] [danimarca_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti Superliga (Danimarca) su {len(DANIMARCA_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(DANIMARCA_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(DANIMARCA_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti Superliga (Danimarca) unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti Superliga (Danimarca) dopo filtro qualita': {len(slugs)}")
