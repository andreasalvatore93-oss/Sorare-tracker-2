"""
danimarca_mid_discovery_global.py

Discovery GLOBALE di tutti i centrocampisti Superliga (Danimarca) in_season attivi (NON solo
...
resta sui soli giocatori posseduti via danimarca_mid_discovery.py).
...
Output: danimarca_mid_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_danimarca/output/danimarca_mid_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [danimarca_mid_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE centrocampisti Superliga (Danimarca) su {len(DANIMARCA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale centrocampisti Superliga (Danimarca) unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale centrocampisti Superliga (Danimarca) dopo filtro qualita': {len(slugs)}")
