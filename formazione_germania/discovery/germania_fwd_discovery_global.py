"""
germania_fwd_discovery_global.py
...
resta sui soli giocatori posseduti via germania_fwd_discovery.py).
...
Output: germania_fwd_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_germania/output/germania_fwd_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [germania_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti Bundesliga su {len(GERMANIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale attaccanti Bundesliga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti Bundesliga dopo filtro qualita': {len(slugs)}")
