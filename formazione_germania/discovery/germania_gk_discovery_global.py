"""
germania_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri Bundesliga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via germania_gk_discovery.py).
...
Output: germania_gk_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_germania/output/germania_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [germania_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri Bundesliga su {len(GERMANIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri Bundesliga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri Bundesliga dopo filtro qualita': {len(slugs)}")
