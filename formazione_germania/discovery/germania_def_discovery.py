"""
Bundesliga (Germania) Defender Discovery
...
lega Bundesliga (Germania) (slug 'bundesliga-de').
...
si tiene solo chi ha domesticLeague.slug == 'bundesliga-de'. Query un po' meno
...
Output: germania_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_germania/output/germania_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Bundesliga (Germania).
TARGET_LEAGUE_SLUG = 'bundesliga-de'
...
def log(msg):
    ...
    print(f"[{ts}] [germania_def_discovery] {msg}")
...
def discover_germania_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Bundesliga (Germania)) posseduti da user_slug, SIA in_season CHE classic. Filtro
...
def main():
    log("Avvio discovery difensori Bundesliga (Germania) (in_season + classic)...")
    slugs, card_counts = discover_germania_defenders_all()
