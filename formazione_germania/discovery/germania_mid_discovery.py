"""
Bundesliga (Germania) Midfielder Discovery
...
Output: germania_mid_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_germania/output/germania_mid_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [germania_mid_discovery] {msg}")
...
def discover_germania_midfielders_all(user_slug=USER_SLUG, max_pages=50):
...
def main():
    log("Avvio discovery centrocampisti Bundesliga (Germania) (in_season + classic)...")
    slugs, card_counts = discover_germania_midfielders_all()
