...
def check_sorare():
    ...
    for target in lista_giocatori:
        ...
        # Query corretta: utilizziamo receiverSide per estrarre gli eurCents
        query = f"""
        query {{
          players(slugs: ["{slug}"]) {{
            ... on Player {{
                ...
                if 'data' in res and res['data']['players'] and res['data']['players'][0]:
                    ...
                    if card and card.get('liveSingleSaleOffer'):
                        ...
                        if receiver_side and receiver_side.get('amounts'):
                            # Estrai il valore in centesimi e converti in euro
                            prezzo_cents = receiver_side['amounts'].get('eurCents')
                            if prezzo_cents is not None:
                                prezzo = float(prezzo_cents) / 100.0
                                print(f"LOG -> {nome} ({tipo}): Prezzo rilevato {prezzo}€ | Soglia {soglia}€")
                                
                                if prezzo <= soglia:
                                    send_email(f"🔔 ALERT: {nome} {tipo}", f"Prezzo: {prezzo}€")
                    else:
                        print(f"LOG -> {nome} ({tipo}): Nessuna offerta attiva trovata.")
        except Exception as e:
