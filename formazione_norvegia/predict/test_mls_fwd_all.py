...
generata da norvegia_fwd_discovery.py) da norvegia_fwd_discovery/player_slugs.json.
...
- Lista giocatori letta dinamicamente da norvegia_fwd_discovery/player_slugs.json
...
# formazione_norvegia/predict/test_mls_fwd_all.py`, quindi l'import diretto
...
DISCOVERY_FILE = os.path.join(
    'formazione_norvegia/output/norvegia_fwd_discovery_global' if CALIBRATION_MODE else 'formazione_norvegia/output/norvegia_fwd_discovery',
    'player_slugs.json')

# Fallback statico SOLO se norvegia_fwd_discovery/player_slugs.json non esiste
...
OUTPUT_DIR = 'formazione_norvegia/output/norvegia_fwd_calibration' if CALIBRATION_MODE else 'formazione_norvegia/output/norvegia_fwd_all'
...
_CIRCUIT_BREAKER_PATH = '/tmp/sorare_cloudfront_block_norvegia_fwd.marker'
...
def rigorous_backtest_prod_fwd(scores, is_home_flags,
                               ...
                               opponent_team_slugs_hist=None, league='norvegia',
                               offensive_values=None,
...
def build_prediction(player_slug):
    ...
    _opp_lambda_mult = opponent_strength.opponent_lambda_multiplier(
        'norvegia', 'fwd', next_opponent_team_slug, _opp_cutoff)
    ...
    _fwd_offense_delta = opponent_strength.fwd_offense_granular_delta(
        'norvegia', next_opponent_team_slug, _opp_cutoff, _offensive_hist)
    grezzo_nuovo = (level_score_atteso + media_granulari_pesata * fattore_trend_granulare
                    + _fwd_offense_delta)
    ...
    score_ordinamento = compute_score_atteso_fwd(
        ...
        use_stadio_d=False,
        next_opponent_team_slug=next_opponent_team_slug, league='norvegia',
        offensive_values=offensive_values,
        detail_ok_flags=detail_ok_flags)
