"""
austria_def_discovery_global.py

Discovery GLOBALE di tutti i difensori Austrian Bundesliga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via austria_def_discovery.py).

Clone di mls_def_discovery_global.py, stesso approccio: per ognuna delle 12
...
Output: austria_def_discovery_global/player_slugs.json -- lista JSON di slug
giocatore difensore unici, deduplicati su tutte le 12 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_austria/output/austria_def_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [austria_def_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE difensori Austrian Bundesliga su {len(AUSTRIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale difensori Austrian Bundesliga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale difensori Austrian Bundesliga dopo filtro qualita': {len(slugs)}")
