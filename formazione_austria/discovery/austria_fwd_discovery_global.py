"""
austria_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti Bundesliga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via austria_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 12
squadre Austrian Bundesliga interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "austrian-bundesliga") { clubs(first: 50) { nodes {
slug name } } } }` -- 12/12 squadre restituite (workflow
verify_clubs.yml, run 30612984333).
...
Output: austria_fwd_discovery_global/player_slugs.json -- lista JSON di slug
giocatore attaccante unici, deduplicati su tutte le 12 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_austria/output/austria_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "austrian-bundesliga")
# .clubs, 31/07) -- 12/12
AUSTRIA_TEAM_SLUGS = [
...
def log(msg):
    ...
    print(f"[{ts}] [austria_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti Austrian Bundesliga su {len(AUSTRIA_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(AUSTRIA_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(AUSTRIA_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti Austrian Bundesliga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti Austrian Bundesliga dopo filtro qualita': {len(slugs)}")
