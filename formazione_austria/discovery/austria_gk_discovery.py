"""
Austrian Bundesliga (Austria) Goalkeeper Discovery
...
Output: austria_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_austria/output/austria_gk_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [austria_gk_discovery] {msg}")
...
def discover_austria_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Austria
    dopo il fetch."""
    ...
    query = """
    query AustriaGoalkeeperDiscovery($userSlug: String!, $page: Int!, $pageSize: Int!,
    ...
    for in_season_value, type_key in (("true", "in_season"), ("false", "classic")):
        ...
        while page <= max_pages:
            data = graphql_query(query, {
                ...
                "refinements": refinements,
            }, operation_name="AustriaGoalkeeperDiscovery")
...
def main():
    log("Avvio discovery portieri Austrian Bundesliga (Austria) (in_season + classic)...")
    slugs, card_counts = discover_austria_goalkeepers_all()
