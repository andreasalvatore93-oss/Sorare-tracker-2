"""
austria_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri Austrian Bundesliga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via austria_gk_discovery.py).
...
Output: austria_gk_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_austria/output/austria_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [austria_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri Austrian Bundesliga su {len(AUSTRIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri Austrian Bundesliga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri Austrian Bundesliga dopo filtro qualita': {len(slugs)}")
