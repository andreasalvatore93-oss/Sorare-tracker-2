"""
austria_mid_discovery_global.py

Discovery GLOBALE di tutti i centrocampisti Austrian Bundesliga in_season attivi (NON solo
...
resta sui soli giocatori posseduti via austria_mid_discovery.py).

Clone di mls_mid_discovery_global.py, stesso approccio: per ognuna delle 12
...
Output: austria_mid_discovery_global/player_slugs.json -- lista JSON di slug
giocatore centrocampiste unici, deduplicati su tutte le 12 squadre, FILTRATI per
qualita'.
"""
import os
import json
...
OUTPUT_DIR = 'formazione_austria/output/austria_mid_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [austria_mid_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE centrocampisti Austrian Bundesliga su {len(AUSTRIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale centrocampisti Austrian Bundesliga unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale centrocampisti Austrian Bundesliga dopo filtro qualita': {len(slugs)}")
