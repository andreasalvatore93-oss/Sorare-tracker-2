"""
italia_fwd_discovery_global.py

Discovery GLOBALE di tutti i attaccanti Serie A in_season attivi (NON solo
...
resta sui soli giocatori posseduti via italia_fwd_discovery.py).

Clone di mls_fwd_discovery_global.py, stesso approccio: per ognuna delle 20
squadre Serie A interroga Club(slug).anyPlayers per il roster completo,
...
`football { competition(slug: "serie-a-it") { clubs(first: 50) { nodes {
...
Output: italia_fwd_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_italia/output/italia_fwd_discovery_global'
TARGET_POSITION = 'Forward'

# Slug ufficiali confermati dal vivo (query Competition(slug: "serie-a-it")
# .clubs, 31/07) -- 20/20
ITALIA_TEAM_SLUGS = [
    'atalanta-ciserano',
    'bologna-bologna',
...
def log(msg):
    ...
    print(f"[{ts}] [italia_fwd_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE attaccanti Serie A su {len(ITALIA_TEAM_SLUGS)} squadre...")

    all_slugs = set()
    all_names = {}
    for idx, team_slug in enumerate(ITALIA_TEAM_SLUGS, 1):
        log(f"[{idx}/{len(ITALIA_TEAM_SLUGS)}] Squadra: {team_slug}")
    ...
    log(f"Totale attaccanti Serie A unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale attaccanti Serie A dopo filtro qualita': {len(slugs)}")
