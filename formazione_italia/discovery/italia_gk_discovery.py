"""
Serie A (Italia) Goalkeeper Discovery
...
Output: italia_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_italia/output/italia_gk_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [italia_gk_discovery] {msg}")
...
def discover_italia_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Italia
    dopo il fetch."""
...
def main():
    log("Avvio discovery portieri Serie A (Italia) (in_season + classic)...")
    slugs, card_counts = discover_italia_goalkeepers_all()
