"""
Serie A (Italia) Forward Discovery
...
Output: italia_fwd_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_italia/output/italia_fwd_discovery'
PAGE_SIZE = 20
...
def log(msg):
    ...
    print(f"[{ts}] [italia_fwd_discovery] {msg}")
...
def discover_italia_forwards_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Forward posseduti (qualunque lega) e scarta quelli non Italia
    dopo il fetch."""
...
def main():
    log("Avvio discovery attaccanti Serie A (Italia) (in_season + classic)...")
    slugs, card_counts = discover_italia_forwards_all()
