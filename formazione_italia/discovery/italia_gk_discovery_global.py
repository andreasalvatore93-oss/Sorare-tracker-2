"""
italia_gk_discovery_global.py

Discovery GLOBALE di tutti i portieri Serie A in_season attivi (NON solo
...
resta sui soli giocatori posseduti via italia_gk_discovery.py).
...
Output: italia_gk_discovery_global/player_slugs.json -- lista JSON di slug
...
OUTPUT_DIR = 'formazione_italia/output/italia_gk_discovery_global'
...
def log(msg):
    ...
    print(f"[{ts}] [italia_gk_discovery_global] {msg}")
...
def main():
    log(f"Avvio discovery GLOBALE portieri Serie A su {len(ITALIA_TEAM_SLUGS)} squadre...")
    ...
    log(f"Totale portieri Serie A unici trovati: {len(slugs)}")

    log(f"Filtro qualita' (media L5/L10/L40 >= {MIN_AVG_SCORE_QUALITY})...")
    slugs = filter_by_quality(slugs)
    log(f"Totale portieri Serie A dopo filtro qualita': {len(slugs)}")
