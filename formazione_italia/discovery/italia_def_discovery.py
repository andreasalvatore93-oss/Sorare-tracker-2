"""
Serie A (Italia) Defender Discovery
...
lega Serie A (Italia) (slug 'serie-a-it').
...
mlspa). Non conosciamo il codice equivalente per Italia (rischio di
...
si tiene solo chi ha domesticLeague.slug == 'serie-a-it'. Query un po' meno
...
Output: italia_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_italia/output/italia_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Serie A (Italia).
TARGET_LEAGUE_SLUG = 'serie-a-it'
...
def log(msg):
    ...
    print(f"[{ts}] [italia_def_discovery] {msg}")
...
def discover_italia_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Serie A (Italia)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Italia
    dopo il fetch."""
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Italia applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Italia unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Italia potrebbero essere
...
def main():
    log("Avvio discovery difensori Serie A (Italia) (in_season + classic)...")
    slugs, card_counts = discover_italia_defenders_all()
