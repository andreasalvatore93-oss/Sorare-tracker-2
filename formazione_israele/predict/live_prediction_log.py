...
su dati storici in cache (formazione_israele/diagnostics/validate_*.py). Non
c'e' nessun tracciamento di quanto le predizioni REALI di produzione (quelle
scritte nei file .txt in israele_<ruolo>_all/) si rivelino accurate una volta
...
formazione_israele/diagnostics/resolve_live_predictions.py si occupa in seguito
...
Uso (da ciascuno dei 4 script predict, SOLO se non CALIBRATION_MODE):
    ...
    from formazione_israele.predict.live_prediction_log import log_live_prediction
