"""
Süper Lig (Turchia) Midfielder Discovery
...
Output: turchia_mid_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_turchia/output/turchia_mid_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [turchia_mid_discovery] {msg}")
...
def discover_turchia_midfielders_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Midfielder posseduti (qualunque lega) e scarta quelli non Turchia
    dopo il fetch."""
...
def main():
    log("Avvio discovery centrocampisti Süper Lig (Turchia) (in_season + classic)...")
    slugs, card_counts = discover_turchia_midfielders_all()
