"""
Süper Lig (Turchia) Defender Discovery
...
lega Süper Lig (Turchia) (slug 'spor-toto-super-lig').
...
mlspa). Non conosciamo il codice equivalente per Turchia (rischio di
...
Output: turchia_def_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_turchia/output/turchia_def_discovery'
PAGE_SIZE = 20

# Lega target -- SOLO Süper Lig (Turchia).
TARGET_LEAGUE_SLUG = 'spor-toto-super-lig'
...
def log(msg):
    ...
    print(f"[{ts}] [turchia_def_discovery] {msg}")
...
def discover_turchia_defenders_all(user_slug=USER_SLUG, max_pages=50):
    """Ritorna la lista deduplicata di slug giocatore (Defender, lega
    Süper Lig (Turchia)) posseduti da user_slug, SIA in_season CHE classic. Filtro
    lega lato CLIENT (vedi docstring modulo) -- nessun filtro
    active_competitions server-side, quindi la query recupera TUTTI i
    Defender posseduti (qualunque lega) e scarta quelli non Turchia
    dopo il fetch."""
        ...
        while page <= max_pages:
            ...
            if page == 1:
                log(f"Totale carte Defender/{type_key} (TUTTE le leghe) trovate: {search.get('nbHits')} "
                    f"({search.get('nbPages')} pagine) -- filtro Turchia applicato dopo il fetch")
            if not hits:
                break
    ...
    log(f"Carte scansionate: {total_card_count} (di cui {total_other_league} altre leghe, scartate) "
        f"| Giocatori Turchia unici (dedup): {len(slugs)}")
...
# kleague_gk_discovery.py. NOTA: i punteggi di Turchia potrebbero essere
...
def main():
    log("Avvio discovery difensori Süper Lig (Turchia) (in_season + classic)...")
    slugs, card_counts = discover_turchia_defenders_all()
