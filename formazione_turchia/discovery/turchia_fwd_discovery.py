"""
Süper Lig (Turchia) Forward Discovery
...
Output: turchia_fwd_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_turchia/output/turchia_fwd_discovery'
...
def log(msg):
    ...
    print(f"[{ts}] [turchia_fwd_discovery] {msg}")
...
def discover_turchia_forwards_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Forward posseduti (qualunque lega) e scarta quelli non Turchia
    dopo il fetch."""
...
def main():
    log("Avvio discovery attaccanti Süper Lig (Turchia) (in_season + classic)...")
    slugs, card_counts = discover_turchia_forwards_all()
