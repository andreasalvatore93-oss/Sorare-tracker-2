"""
Süper Lig (Turchia) Goalkeeper Discovery
...
Output: turchia_gk_discovery/player_slugs.json -- lista JSON di slug
...
USER_SLUG = 'crowss'
OUTPUT_DIR = 'formazione_turchia/output/turchia_gk_discovery'
...
def discover_turchia_goalkeepers_all(user_slug=USER_SLUG, max_pages=50):
    ...
    Goalkeeper posseduti (qualunque lega) e scarta quelli non Turchia
    dopo il fetch."""
...
def main():
    log("Avvio discovery portieri Süper Lig (Turchia) (in_season + classic)...")
    slugs, card_counts = discover_turchia_goalkeepers_all()
